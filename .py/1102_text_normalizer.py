...
STATE = BASE / "production_state"; REPORTS = BASE / "raporlar"; MODULE_DIR = STATE / "decision_processing_pipeline" / "1102_text_normalizer"
MODULE_ID = "1102"; MODULE_NAME = "Text Normalizer"
...
def main():
    ...
    decision = "TEXT NORMALIZER READY" if not val["errors"] else "TEXT NORMALIZER BLOCKED"
    ...
    ts = now_stamp(); output = MODULE_DIR / "1102_text_normalizer.json"; state = MODULE_DIR / ("1102_text_normalizer_state_" + ts + ".json"); report = REPORTS / ("1102_text_normalizer_raporu_" + ts + ".txt")
