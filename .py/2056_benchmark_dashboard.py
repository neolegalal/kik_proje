...
MODULE_DIR = STATE / "validation_benchmark_platform" / "2056_benchmark_dashboard"
MODULE_ID = "2056"
MODULE_NAME = "Benchmark Dashboard"
def now_stamp():
...
def main():
    ...
    decision = "BENCHMARK DASHBOARD READY" if not val["errors"] else "BENCHMARK DASHBOARD BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "2056_benchmark_dashboard.json"
    state = MODULE_DIR / ("2056_benchmark_dashboard_state_" + ts + ".json")
    report = REPORTS / ("2056_benchmark_dashboard_raporu_" + ts + ".txt")
