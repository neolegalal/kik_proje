...
FACTORY_DIR = STATE / "production_data_factory" / "508_data_factory_dashboard"
MODULE_ID = "508"
MODULE_NAME = "Data Factory Dashboard"
...
def analyze(payload):
    ...
    decision='DATA FACTORY DASHBOARD READY' if not validation.get('errors') else 'DATA FACTORY DASHBOARD REVIEW'
...
def main():
    ...
    ts=now_stamp(); output=FACTORY_DIR/('508_data_factory_dashboard.json'); state=FACTORY_DIR/('508_data_factory_dashboard_state_'+ts+'.json'); report=REPORTS/('508_data_factory_dashboard_raporu_'+ts+'.txt')
