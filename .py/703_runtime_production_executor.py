...
PILOT_MODULE_DIR = STATE / "pilot_production" / "703_runtime_production_executor"
MODULE_ID = "703"
MODULE_NAME = "Runtime Production Executor"
...
def analyze(payload):
    ...
    decision='RUNTIME PRODUCTION EXECUTOR READY' if not validation.get('errors') else 'RUNTIME PRODUCTION EXECUTOR REVIEW'
...
def main():
    ...
    ts=now_stamp(); output=PILOT_MODULE_DIR/('703_runtime_production_executor.json'); state=PILOT_MODULE_DIR/('703_runtime_production_executor_state_'+ts+'.json'); report=REPORTS/('703_runtime_production_executor_raporu_'+ts+'.txt')
