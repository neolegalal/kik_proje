...
MODULE_DIR = STATE / "unified_decision_processor" / "1955_master_record_builder"
MODULE_ID = "1955"
MODULE_NAME = "Master Record Builder"
def now_stamp():
...
def main():
    ...
    decision = "MASTER RECORD BUILDER READY" if not val["errors"] else "MASTER RECORD BUILDER BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1955_master_record_builder.json"
    state = MODULE_DIR / ("1955_master_record_builder_state_" + ts + ".json")
    report = REPORTS / ("1955_master_record_builder_raporu_" + ts + ".txt")
