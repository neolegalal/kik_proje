...
MODULE_DIR = STATE / "litigation_intelligence" / "1407_stay_of_execution_analyzer"
MODULE_ID = "1407"
MODULE_NAME = "Stay Of Execution Analyzer"
def now_stamp():
...
def main():
    ...
    decision = "STAY OF EXECUTION ANALYZER READY" if not val["errors"] else "STAY OF EXECUTION ANALYZER BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1407_stay_of_execution_analyzer.json"
    state = MODULE_DIR / ("1407_stay_of_execution_analyzer_state_" + ts + ".json")
    report = REPORTS / ("1407_stay_of_execution_analyzer_raporu_" + ts + ".txt")
