...
MODULE_DIR = STATE / "legal_advisory_intelligence" / "1301_case_intake_analyzer"
MODULE_ID = "1301"
MODULE_NAME = "Case Intake Analyzer"
...
def main():
    ...
    decision = "CASE INTAKE ANALYZER READY" if not val["errors"] else "CASE INTAKE ANALYZER BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1301_case_intake_analyzer.json"
    state = MODULE_DIR / ("1301_case_intake_analyzer_state_" + ts + ".json")
    report = REPORTS / ("1301_case_intake_analyzer_raporu_" + ts + ".txt")
