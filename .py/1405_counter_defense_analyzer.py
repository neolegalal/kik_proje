...
MODULE_DIR = STATE / "litigation_intelligence" / "1405_counter_defense_analyzer"
MODULE_ID = "1405"
MODULE_NAME = "Counter Defense Analyzer"
def now_stamp():
...
def main():
    ...
    decision = "COUNTER DEFENSE ANALYZER READY" if not val["errors"] else "COUNTER DEFENSE ANALYZER BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1405_counter_defense_analyzer.json"
    state = MODULE_DIR / ("1405_counter_defense_analyzer_state_" + ts + ".json")
    report = REPORTS / ("1405_counter_defense_analyzer_raporu_" + ts + ".txt")
