...
MODULE_DIR=BASE/"production_state"/"neolegal_legal_brain"/"2104_judge_simulation"
...
def main():
    ...
    r=NeoLegalLegalBrainSDK(name="2104 Judge Simulation",case_text=a.case_text,master_record_path=a.master_record,execute=a.execute).run()
    v=r["payload"]["validation"]; b=r["payload"]["brain_state"]; audit=r["payload"]["audit"]
    decision="JUDGE SIMULATION READY" if not v["errors"] else "JUDGE SIMULATION BLOCKED"
    ...
    stamp=ts(); out=MODULE_DIR/"2104_judge_simulation.json"; state=MODULE_DIR/("2104_judge_simulation_state_"+stamp+".json"); rep=REPORTS/("2104_judge_simulation_raporu_"+stamp+".txt")
    payload={"module_id":"2104","module_name":"Judge Simulation","analysis":analysis,"sdk_reference":r["paths"]}
    ...
    lines=["="*80,"2104 JUDGE SIMULATION","="*80,"Score                 : "+str(analysis["score"])+" / 100","Decision              : "+decision,"Thinking Score        : "+str(analysis["thinking_score"])+" / 100","Base Probability      : "+str(analysis["base_probability"])+" / 100","Judge View            : "+str(analysis["judge_view"]),"Reflection Confidence : "+str(analysis["reflection_confidence"])+" / 100","Audit                 : "+str(analysis["audit"])]
