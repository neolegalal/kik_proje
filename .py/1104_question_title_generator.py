...
STATE = BASE / "production_state"; REPORTS = BASE / "raporlar"; MODULE_DIR = STATE / "decision_processing_pipeline" / "1104_question_title_generator"
MODULE_ID = "1104"; MODULE_NAME = "Question Title Generator"
...
def main():
    ...
    decision = "QUESTION TITLE GENERATOR READY" if not val["errors"] else "QUESTION TITLE GENERATOR BLOCKED"
    ...
    ts = now_stamp(); output = MODULE_DIR / "1104_question_title_generator.json"; state = MODULE_DIR / ("1104_question_title_generator_state_" + ts + ".json"); report = REPORTS / ("1104_question_title_generator_raporu_" + ts + ".txt")
