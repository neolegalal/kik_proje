...
IPE_DIR = STATE / "intelligent_production_engine" / "650_production_analytics_center"
MODULE_ID = "650"
MODULE_NAME = "Production Analytics Center"
...
def analyze(payload):
    ...
    decision='PRODUCTION ANALYTICS CENTER READY' if not validation.get('errors') else 'PRODUCTION ANALYTICS CENTER REVIEW'
...
def main():
    ...
    ts=now_stamp(); output=IPE_DIR/('650_production_analytics_center.json'); state=IPE_DIR/('650_production_analytics_center_state_'+ts+'.json'); report=REPORTS/('650_production_analytics_center_raporu_'+ts+'.txt')
