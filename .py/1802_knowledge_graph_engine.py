...
MODULE_DIR = STATE / "next_generation_neolegal" / "1802_knowledge_graph_engine"
MODULE_ID = "1802"
MODULE_NAME = "Knowledge Graph Engine"
def now_stamp():
...
def main():
    ...
    decision = "KNOWLEDGE GRAPH ENGINE READY" if not val["errors"] else "KNOWLEDGE GRAPH ENGINE BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1802_knowledge_graph_engine.json"
    state = MODULE_DIR / ("1802_knowledge_graph_engine_state_" + ts + ".json")
    report = REPORTS / ("1802_knowledge_graph_engine_raporu_" + ts + ".txt")
