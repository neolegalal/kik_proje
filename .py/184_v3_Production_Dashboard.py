...
def build_dashboard():
    ...
    prefixes = {
        "181": "181_v4_final_master_controller_state_",
    ...
    # 181 v4 yoksa eski 181 v3 state'e düş.
    if not states["181"]["state"]:
        path, state = latest_state("181_v3_final_master_controller_state_")
    ...
    html_path = os.path.join(DASHBOARD_DIR, f"184_v3_production_dashboard_{run_tag}.html")
    txt_path = os.path.join(RAPOR_DIR, f"184_v3_production_dashboard_raporu_{run_tag}.txt")
    state_path = os.path.join(STATE_DIR, f"184_v3_production_dashboard_state_{run_tag}.json")

    metrics = ""
    metrics += metric_html("Final Üretim Durumu", "EVET" if final_ready else "HAYIR", "181 v4 sonucu", "ok" if final_ready else "bad")
...
<head>
<meta charset="utf-8">
<title>184 v3 Production Dashboard</title>
<style>
...
<body>
<div class="header">
  <h1>184 v3 Production Dashboard</h1>
...
{section("State Dosyaları", table_rows(state_rows))}
    ...
    with open(txt_path, "w", encoding="utf-8") as f:
        f.write("184 v3 - PRODUCTION DASHBOARD RAPORU\n")
        f.write("=" * 80 + "\n\n")
...
def main():
    print("=" * 80)
    print("184 v3 - PRODUCTION DASHBOARD")
    print("=" * 80)

    html_path, txt_path, state_path, st = build_dashboard()

    print("\n184 v3 PRODUCTION DASHBOARD TAMAMLANDI")
    print("-" * 80)
