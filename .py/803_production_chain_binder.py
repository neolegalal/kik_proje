...
MODULE_DIR = STATE / "real_production_engine" / "803_production_chain_binder"
MODULE_ID = "803"
MODULE_NAME = "Production Chain Binder"
...
def analyze(payload):
    ...
    decision='PRODUCTION CHAIN BINDER READY' if not validation.get('errors') else 'PRODUCTION CHAIN BINDER REVIEW'
...
def main():
    ...
    ts=now_stamp(); output=MODULE_DIR/('803_production_chain_binder.json'); state=MODULE_DIR/('803_production_chain_binder_state_'+ts+'.json'); report=REPORTS/('803_production_chain_binder_raporu_'+ts+'.txt')
