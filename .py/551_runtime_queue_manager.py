...
RUNTIME_DIR = STATE / "production_factory_runtime" / "551_runtime_queue_manager"
MODULE_ID = "551"
MODULE_NAME = "Runtime Queue Manager"
...
def analyze(payload):
    ...
    decision='RUNTIME QUEUE MANAGER READY' if not validation.get('errors') else 'RUNTIME QUEUE MANAGER REVIEW'
...
def main():
    ...
    ts=now_stamp(); output=RUNTIME_DIR/('551_runtime_queue_manager.json'); state=RUNTIME_DIR/('551_runtime_queue_manager_state_'+ts+'.json'); report=REPORTS/('551_runtime_queue_manager_raporu_'+ts+'.txt')
