...
OS_DIR = STATE / "production_os" / "410_production_manager"
MODULE_ID = "410"
MODULE_NAME = "Production Manager"
...
def analyze(payload):
    ...
    decision = 'PRODUCTION MANAGER READY' if not validation.get('errors') else 'PRODUCTION MANAGER REVIEW'
...
def main():
    ...
    ts = now_stamp()
    output = OS_DIR / ('410_production_manager.json')
    state = OS_DIR / ('410_production_manager_state_' + ts + '.json')
    report = REPORTS / ('410_production_manager_raporu_' + ts + '.txt')
