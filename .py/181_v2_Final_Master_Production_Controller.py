# -*- coding: utf-8 -*-
"""
181 v2 - FINAL MASTER PRODUCTION CONTROLLER

v2 düzeltmeleri:
- Her adımın OK/FAIL gerekçesini ekrana basar.
- 171 v2 state alanlarını daha esnek okur.
- 171'in "can_go_169=False ama block=0 score yüksek" durumunu başarısız saymaz.
- 173 v2 non-blocking kalır.
"""
...
def step_ok_171():
    ...
    block = as_int(get_any(s, ["block_cards", "block_count", "BLOCK kart", "block"], 0))
    avg = as_float(get_any(s, ["avg_score", "average_score", "ortalama_puan", "average"], 0))
    ...
    # v2: asıl sert şart BLOCK=0. Score varsa 85 üstü kabul.
...
def step_ok_180():
    ...
    ok = (ready or avg >= 70) and err == 0 and too_few == 0 and fail == 0
    return ok, f"ready={ready} avg={avg} too_few={too_few} fail={fail} error={err}"
...
def main():
    print("=" * 80)
    print("181 v2 - FINAL MASTER PRODUCTION CONTROLLER")
    print("=" * 80)

    run_tag = tag()
    limit, flags = parse_args()

    log_path = os.path.join(LOG_DIR, f"181_v2_final_master_controller_log_{run_tag}.txt")
    state_path = os.path.join(STATE_DIR, f"181_v2_final_master_controller_state_{run_tag}.json")
    rapor_path = os.path.join(RAPOR_DIR, f"181_v2_final_master_controller_raporu_{run_tag}.txt")
    ...
    with open(rapor_path, "w", encoding="utf-8") as f:
        f.write("181 v2 - FINAL MASTER PRODUCTION CONTROLLER RAPORU\n")
        f.write("=" * 80 + "\n\n")
    ...
    print("\n181 v2 FINAL MASTER CONTROLLER TAMAMLANDI")
    print("-" * 80)
