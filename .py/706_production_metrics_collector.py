...
PILOT_MODULE_DIR = STATE / "pilot_production" / "706_production_metrics_collector"
MODULE_ID = "706"
MODULE_NAME = "Production Metrics Collector"
...
def analyze(payload):
    ...
    decision='PRODUCTION METRICS COLLECTOR READY' if not validation.get('errors') else 'PRODUCTION METRICS COLLECTOR REVIEW'
...
def main():
    ...
    ts=now_stamp(); output=PILOT_MODULE_DIR/('706_production_metrics_collector.json'); state=PILOT_MODULE_DIR/('706_production_metrics_collector_state_'+ts+'.json'); report=REPORTS/('706_production_metrics_collector_raporu_'+ts+'.txt')
