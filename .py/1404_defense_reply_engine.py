...
MODULE_DIR = STATE / "litigation_intelligence" / "1404_defense_reply_engine"
MODULE_ID = "1404"
MODULE_NAME = "Defense Reply Engine"
def now_stamp():
...
def main():
    ...
    decision = "DEFENSE REPLY ENGINE READY" if not val["errors"] else "DEFENSE REPLY ENGINE BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1404_defense_reply_engine.json"
    state = MODULE_DIR / ("1404_defense_reply_engine_state_" + ts + ".json")
    report = REPORTS / ("1404_defense_reply_engine_raporu_" + ts + ".txt")
