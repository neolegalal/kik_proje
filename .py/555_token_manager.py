...
RUNTIME_DIR = STATE / "production_factory_runtime" / "555_token_manager"
MODULE_ID = "555"
MODULE_NAME = "Token Manager"
...
def analyze(payload):
    ...
    decision='TOKEN MANAGER READY' if not validation.get('errors') else 'TOKEN MANAGER REVIEW'
...
def main():
    ...
    ts=now_stamp(); output=RUNTIME_DIR/('555_token_manager.json'); state=RUNTIME_DIR/('555_token_manager_state_'+ts+'.json'); report=REPORTS/('555_token_manager_raporu_'+ts+'.txt')
