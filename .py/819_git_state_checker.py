...
MODULE_DIR = STATE / "production_safety_gate" / "819_git_state_checker"
MODULE_ID = "819"
MODULE_NAME = "Git State Checker"
...
def main():
    ...
    decision='GIT STATE CHECKER READY' if final['decision'] in ('PASS','WARN') else 'GIT STATE CHECKER BLOCKED'
    ...
    ts=now_stamp(); output=MODULE_DIR/('819_git_state_checker.json'); state=MODULE_DIR/('819_git_state_checker_state_'+ts+'.json'); report=REPORTS/('819_git_state_checker_raporu_'+ts+'.txt')
