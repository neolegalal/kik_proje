...
MODULE_DIR = STATE / "production_safety_gate" / "816_backup_rollback_checker"
MODULE_ID = "816"
MODULE_NAME = "Backup Rollback Checker"
...
def main():
    ...
    decision='BACKUP ROLLBACK CHECKER READY' if final['decision'] in ('PASS','WARN') else 'BACKUP ROLLBACK CHECKER BLOCKED'
    ...
    ts=now_stamp(); output=MODULE_DIR/('816_backup_rollback_checker.json'); state=MODULE_DIR/('816_backup_rollback_checker_state_'+ts+'.json'); report=REPORTS/('816_backup_rollback_checker_raporu_'+ts+'.txt')
