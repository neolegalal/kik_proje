...
MODULE_DIR = STATE / "next_generation_neolegal" / "1803_legal_time_machine"
MODULE_ID = "1803"
MODULE_NAME = "Legal Time Machine"
def now_stamp():
...
def main():
    ...
    decision = "LEGAL TIME MACHINE READY" if not val["errors"] else "LEGAL TIME MACHINE BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1803_legal_time_machine.json"
    state = MODULE_DIR / ("1803_legal_time_machine_state_" + ts + ".json")
    report = REPORTS / ("1803_legal_time_machine_raporu_" + ts + ".txt")
