...
def konu_zorunlu_eslesme(sorgu, kart):
    ...
    ana_alan = " ".join([baslik, anahtar, soru])
    ...
    if len(sorgu_kelimeleri) == 1:
        return sorgu_kelimeleri[0] in ana_alan

    eslesen = sum(1 for k in sorgu_kelimeleri if k in ana_alan)
...
def yazdir(sonuclar):
    print("\n" + "=" * 80)
    print("KONU FİLTRELİ EMSAL SONUÇLARI")
    print("=" * 80)

    if not sonuclar:
        print("Sonuç bulunamadı veya konu filtresini geçemedi.")
        return
...
def main():
    print("=" * 80)
    print("117 - KİK KARAR AI ARAMA MOTORU V4 / KONU ZORUNLU FİLTRE")
    print("=" * 80)
