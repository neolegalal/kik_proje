...
FACTORY_DIR = STATE / "production_data_factory" / "504_production_queue_builder"
MODULE_ID = "504"
MODULE_NAME = "Production Queue Builder"
...
def analyze(payload):
    ...
    decision='PRODUCTION QUEUE BUILDER READY' if not validation.get('errors') else 'PRODUCTION QUEUE BUILDER REVIEW'
...
def main():
    ...
    ts=now_stamp(); output=FACTORY_DIR/('504_production_queue_builder.json'); state=FACTORY_DIR/('504_production_queue_builder_state_'+ts+'.json'); report=REPORTS/('504_production_queue_builder_raporu_'+ts+'.txt')
