...
AUDIT_DIR = STATE / "production_readiness" / '315_database_integrity_auditor'
MODULE_ID = '315'
MODULE_NAME = 'Database Integrity Auditor'
VERSION = 'v2.22'
TAG = 'v2.22-database-integrity-auditor'
MUST_HAVE = ['169']
PATTERNS = ['db', 'sqlite', 'kik.db', 'import', 'migration']
...
def create_docs():
    ...
    release_path = RELEASES / 'v2.22-database-integrity-auditor.md'
...
def main():
    ...
    ts = now_stamp()
    state_path = AUDIT_DIR / (MODULE_ID + "_database_integrity_auditor_audit_" + ts + ".json")
    report_path = REPORTS / (MODULE_ID + "_database_integrity_auditor_raporu_" + ts + ".txt")
