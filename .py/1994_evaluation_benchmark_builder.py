...
MODULE_DIR = STATE / "case_graph_trainer" / "1994_evaluation_benchmark_builder"
MODULE_ID = "1994"
MODULE_NAME = "Evaluation Benchmark Builder"
def now_stamp():
...
def main():
    ...
    decision = "EVALUATION BENCHMARK BUILDER READY" if not val["errors"] else "EVALUATION BENCHMARK BUILDER BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1994_evaluation_benchmark_builder.json"
    state = MODULE_DIR / ("1994_evaluation_benchmark_builder_state_" + ts + ".json")
    report = REPORTS / ("1994_evaluation_benchmark_builder_raporu_" + ts + ".txt")
