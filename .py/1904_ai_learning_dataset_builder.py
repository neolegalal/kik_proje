...
MODULE_DIR = STATE / "neolegal_evolution_platform" / "1904_ai_learning_dataset_builder"
MODULE_ID = "1904"
MODULE_NAME = "AI Learning Dataset Builder"
def now_stamp():
...
def main():
    ...
    decision = "AI LEARNING DATASET BUILDER READY" if not val["errors"] else "AI LEARNING DATASET BUILDER BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1904_ai_learning_dataset_builder.json"
    state = MODULE_DIR / ("1904_ai_learning_dataset_builder_state_" + ts + ".json")
    report = REPORTS / ("1904_ai_learning_dataset_builder_raporu_" + ts + ".txt")
