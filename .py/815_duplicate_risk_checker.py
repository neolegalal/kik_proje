...
MODULE_DIR = STATE / "production_safety_gate" / "815_duplicate_risk_checker"
MODULE_ID = "815"
MODULE_NAME = "Duplicate Risk Checker"
...
def main():
    ...
    decision='DUPLICATE RISK CHECKER READY' if final['decision'] in ('PASS','WARN') else 'DUPLICATE RISK CHECKER BLOCKED'
    ...
    ts=now_stamp(); output=MODULE_DIR/('815_duplicate_risk_checker.json'); state=MODULE_DIR/('815_duplicate_risk_checker_state_'+ts+'.json'); report=REPORTS/('815_duplicate_risk_checker_raporu_'+ts+'.txt')
