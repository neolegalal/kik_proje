...
AUDIT_DIR = STATE / "production_readiness" / '317_load_performance_validator'
MODULE_ID = '317'
MODULE_NAME = 'Load & Performance Validator'
VERSION = 'v2.24'
TAG = 'v2.24-load-performance-validator'
MUST_HAVE = ['190', '195', '207']
PATTERNS = ['runtime', 'monitor', 'worker', 'queue', 'batch', 'performance']
...
def create_docs():
    ...
    release_path = RELEASES / 'v2.24-load-performance-validator.md'
...
def main():
    ...
    ts = now_stamp()
    state_path = AUDIT_DIR / (MODULE_ID + "_load_performance_validator_audit_" + ts + ".json")
    report_path = REPORTS / (MODULE_ID + "_load_performance_validator_raporu_" + ts + ".txt")
