...
FACTORY_DIR = STATE / "production_data_factory" / "506_parallel_production_controller"
MODULE_ID = "506"
MODULE_NAME = "Parallel Production Controller"
...
def analyze(payload):
    ...
    decision='PARALLEL PRODUCTION CONTROLLER READY' if not validation.get('errors') else 'PARALLEL PRODUCTION CONTROLLER REVIEW'
...
def main():
    ...
    ts=now_stamp(); output=FACTORY_DIR/('506_parallel_production_controller.json'); state=FACTORY_DIR/('506_parallel_production_controller_state_'+ts+'.json'); report=REPORTS/('506_parallel_production_controller_raporu_'+ts+'.txt')
