...
PILOT_MODULE_DIR = STATE / "pilot_production" / "707_pilot_report_generator"
MODULE_ID = "707"
MODULE_NAME = "Pilot Report Generator"
...
def analyze(payload):
    ...
    decision='PILOT REPORT GENERATOR READY' if not validation.get('errors') else 'PILOT REPORT GENERATOR REVIEW'
...
def main():
    ...
    ts=now_stamp(); output=PILOT_MODULE_DIR/('707_pilot_report_generator.json'); state=PILOT_MODULE_DIR/('707_pilot_report_generator_state_'+ts+'.json'); report=REPORTS/('707_pilot_report_generator_raporu_'+ts+'.txt')
