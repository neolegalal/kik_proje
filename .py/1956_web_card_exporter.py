...
MODULE_DIR = STATE / "unified_decision_processor" / "1956_web_card_exporter"
MODULE_ID = "1956"
MODULE_NAME = "Web Card Exporter"
def now_stamp():
...
def main():
    ...
    decision = "WEB CARD EXPORTER READY" if not val["errors"] else "WEB CARD EXPORTER BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1956_web_card_exporter.json"
    state = MODULE_DIR / ("1956_web_card_exporter_state_" + ts + ".json")
    report = REPORTS / ("1956_web_card_exporter_raporu_" + ts + ".txt")
