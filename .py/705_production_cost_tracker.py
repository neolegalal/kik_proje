...
PILOT_MODULE_DIR = STATE / "pilot_production" / "705_production_cost_tracker"
MODULE_ID = "705"
MODULE_NAME = "Production Cost Tracker"
...
def analyze(payload):
    ...
    decision='PRODUCTION COST TRACKER READY' if not validation.get('errors') else 'PRODUCTION COST TRACKER REVIEW'
...
def main():
    ...
    ts=now_stamp(); output=PILOT_MODULE_DIR/('705_production_cost_tracker.json'); state=PILOT_MODULE_DIR/('705_production_cost_tracker_state_'+ts+'.json'); report=REPORTS/('705_production_cost_tracker_raporu_'+ts+'.txt')
