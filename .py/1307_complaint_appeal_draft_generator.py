...
MODULE_DIR = STATE / "legal_advisory_intelligence" / "1307_complaint_appeal_draft_generator"
MODULE_ID = "1307"
MODULE_NAME = "Complaint Appeal Draft Generator"
...
def main():
    ...
    decision = "COMPLAINT APPEAL DRAFT GENERATOR READY" if not val["errors"] else "COMPLAINT APPEAL DRAFT GENERATOR BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1307_complaint_appeal_draft_generator.json"
    state = MODULE_DIR / ("1307_complaint_appeal_draft_generator_state_" + ts + ".json")
    report = REPORTS / ("1307_complaint_appeal_draft_generator_raporu_" + ts + ".txt")
