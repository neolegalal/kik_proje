...
MODULE_DIR = STATE / "validation_benchmark_platform" / "2052_benchmark_runner"
MODULE_ID = "2052"
...
def main():
    ...
    decision = "BENCHMARK RUNNER READY" if not val["errors"] else "BENCHMARK RUNNER BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "2052_benchmark_runner.json"
    state = MODULE_DIR / ("2052_benchmark_runner_state_" + ts + ".json")
    report = REPORTS / ("2052_benchmark_runner_raporu_" + ts + ".txt")
