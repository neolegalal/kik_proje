...
MODULE_DIR = STATE / "real_production_engine" / "807_real_production_metrics"
MODULE_ID = "807"
MODULE_NAME = "Real Production Metrics"
...
def analyze(payload):
    ...
    decision='REAL PRODUCTION METRICS READY' if not validation.get('errors') else 'REAL PRODUCTION METRICS REVIEW'
...
def main():
    ...
    ts=now_stamp(); output=MODULE_DIR/('807_real_production_metrics.json'); state=MODULE_DIR/('807_real_production_metrics_state_'+ts+'.json'); report=REPORTS/('807_real_production_metrics_raporu_'+ts+'.txt')
