...
MODULE_DIR = STATE / "neolegal_expert_orchestrator" / "1608_action_plan_generator"
MODULE_ID = "1608"
MODULE_NAME = "Action Plan Generator"
def now_stamp():
...
def main():
    ...
    decision = "ACTION PLAN GENERATOR READY" if not val["errors"] else "ACTION PLAN GENERATOR BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1608_action_plan_generator.json"
    state = MODULE_DIR / ("1608_action_plan_generator_state_" + ts + ".json")
    report = REPORTS / ("1608_action_plan_generator_raporu_" + ts + ".txt")
