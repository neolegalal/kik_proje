...
MODULE_DIR = STATE / "next_generation_neolegal" / "1809_next_generation_release_manager"
MODULE_ID = "1809"
MODULE_NAME = "Next Generation Release Manager"
def now_stamp():
...
def main():
    ...
    decision = "NEXT GENERATION RELEASE MANAGER READY" if not val["errors"] else "NEXT GENERATION RELEASE MANAGER BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1809_next_generation_release_manager.json"
    state = MODULE_DIR / ("1809_next_generation_release_manager_state_" + ts + ".json")
    report = REPORTS / ("1809_next_generation_release_manager_raporu_" + ts + ".txt")
