...
MODULE_DIR = STATE / "case_graph_trainer" / "1991_instruction_dataset_builder"
MODULE_ID = "1991"
MODULE_NAME = "Instruction Dataset Builder"
def now_stamp():
...
def main():
    ...
    decision = "INSTRUCTION DATASET BUILDER READY" if not val["errors"] else "INSTRUCTION DATASET BUILDER BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1991_instruction_dataset_builder.json"
    state = MODULE_DIR / ("1991_instruction_dataset_builder_state_" + ts + ".json")
    report = REPORTS / ("1991_instruction_dataset_builder_raporu_" + ts + ".txt")
