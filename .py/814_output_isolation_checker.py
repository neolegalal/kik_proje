...
MODULE_DIR = STATE / "production_safety_gate" / "814_output_isolation_checker"
MODULE_ID = "814"
MODULE_NAME = "Output Isolation Checker"
...
def main():
    ...
    decision='OUTPUT ISOLATION CHECKER READY' if final['decision'] in ('PASS','WARN') else 'OUTPUT ISOLATION CHECKER BLOCKED'
    ...
    ts=now_stamp(); output=MODULE_DIR/('814_output_isolation_checker.json'); state=MODULE_DIR/('814_output_isolation_checker_state_'+ts+'.json'); report=REPORTS/('814_output_isolation_checker_raporu_'+ts+'.txt')
