...
RUNTIME_DIR = STATE / "production_factory_runtime" / "553_multi_api_runtime"
MODULE_ID = "553"
MODULE_NAME = "Multi API Runtime"
...
def analyze(payload):
    ...
    decision='MULTI API RUNTIME READY' if not validation.get('errors') else 'MULTI API RUNTIME REVIEW'
...
def main():
    ...
    ts=now_stamp(); output=RUNTIME_DIR/('553_multi_api_runtime.json'); state=RUNTIME_DIR/('553_multi_api_runtime_state_'+ts+'.json'); report=REPORTS/('553_multi_api_runtime_raporu_'+ts+'.txt')
