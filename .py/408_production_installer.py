...
OS_DIR = STATE / "production_os" / "408_production_installer"
MODULE_ID = "408"
MODULE_NAME = "Production Installer"
...
def analyze(payload):
    ...
    decision = 'PRODUCTION INSTALLER READY' if not validation.get('errors') else 'PRODUCTION INSTALLER REVIEW'
...
def main():
    ...
    ts = now_stamp()
    output = OS_DIR / ('408_production_installer.json')
    state = OS_DIR / ('408_production_installer_state_' + ts + '.json')
    report = REPORTS / ('408_production_installer_raporu_' + ts + '.txt')
