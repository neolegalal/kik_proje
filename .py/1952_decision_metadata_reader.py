...
MODULE_DIR = STATE / "unified_decision_processor" / "1952_decision_metadata_reader"
MODULE_ID = "1952"
MODULE_NAME = "Decision Metadata Reader"
def now_stamp():
...
def main():
    ...
    decision = "DECISION METADATA READER READY" if not val["errors"] else "DECISION METADATA READER BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1952_decision_metadata_reader.json"
    state = MODULE_DIR / ("1952_decision_metadata_reader_state_" + ts + ".json")
    report = REPORTS / ("1952_decision_metadata_reader_raporu_" + ts + ".txt")
