...
MODULE_DIR = STATE / "case_graph_trainer" / "1993_fine_tuning_dataset_builder"
MODULE_ID = "1993"
MODULE_NAME = "Fine Tuning Dataset Builder"
def now_stamp():
...
def main():
    ...
    decision = "FINE TUNING DATASET BUILDER READY" if not val["errors"] else "FINE TUNING DATASET BUILDER BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1993_fine_tuning_dataset_builder.json"
    state = MODULE_DIR / ("1993_fine_tuning_dataset_builder_state_" + ts + ".json")
    report = REPORTS / ("1993_fine_tuning_dataset_builder_raporu_" + ts + ".txt")
