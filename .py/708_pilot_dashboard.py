...
PILOT_MODULE_DIR = STATE / "pilot_production" / "708_pilot_dashboard"
MODULE_ID = "708"
MODULE_NAME = "Pilot Dashboard"
...
def analyze(payload):
    ...
    decision='PILOT DASHBOARD READY' if not validation.get('errors') else 'PILOT DASHBOARD REVIEW'
...
def main():
    ...
    ts=now_stamp(); output=PILOT_MODULE_DIR/('708_pilot_dashboard.json'); state=PILOT_MODULE_DIR/('708_pilot_dashboard_state_'+ts+'.json'); report=REPORTS/('708_pilot_dashboard_raporu_'+ts+'.txt')
