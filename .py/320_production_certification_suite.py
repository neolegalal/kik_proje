...
AUDIT_DIR = STATE / "production_readiness" / '320_production_certification_suite'
MODULE_ID = '320'
MODULE_NAME = 'Production Certification Suite'
VERSION = 'v2.27'
TAG = 'v2.27-production-certification-suite'
MUST_HAVE = ['310', '311', '312', '313', '314', '315', '316', '317', '318', '319']
PATTERNS = ['summary', 'maturity', 'pipeline', 'readiness', 'certification']
...
def create_docs():
    ...
    release_path = RELEASES / 'v2.27-production-certification-suite.md'
...
def main():
    ...
    ts = now_stamp()
    state_path = AUDIT_DIR / (MODULE_ID + "_production_certification_suite_audit_" + ts + ".json")
    report_path = REPORTS / (MODULE_ID + "_production_certification_suite_raporu_" + ts + ".txt")
