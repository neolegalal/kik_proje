...
MODULE_DIR = STATE / "validation_benchmark_platform" / "2051_gold_standard_builder"
MODULE_ID = "2051"
MODULE_NAME = "Gold Standard Builder"
def now_stamp():
...
def main():
    ...
    decision = "GOLD STANDARD BUILDER READY" if not val["errors"] else "GOLD STANDARD BUILDER BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "2051_gold_standard_builder.json"
    state = MODULE_DIR / ("2051_gold_standard_builder_state_" + ts + ".json")
    report = REPORTS / ("2051_gold_standard_builder_raporu_" + ts + ".txt")
