...
MODULE_DIR = STATE / "next_generation_neolegal" / "1810_next_generation_certificate"
MODULE_ID = "1810"
MODULE_NAME = "Next Generation Certificate"
def now_stamp():
...
def main():
    ...
    decision = "NEXT GENERATION CERTIFICATE READY" if not val["errors"] else "NEXT GENERATION CERTIFICATE BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1810_next_generation_certificate.json"
    state = MODULE_DIR / ("1810_next_generation_certificate_state_" + ts + ".json")
    report = REPORTS / ("1810_next_generation_certificate_raporu_" + ts + ".txt")
