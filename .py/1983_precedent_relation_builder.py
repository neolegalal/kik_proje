...
MODULE_DIR = STATE / "case_graph_trainer" / "1983_precedent_relation_builder"
MODULE_ID = "1983"
MODULE_NAME = "Precedent Relation Builder"
def now_stamp():
...
def main():
    ...
    decision = "PRECEDENT RELATION BUILDER READY" if not val["errors"] else "PRECEDENT RELATION BUILDER BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1983_precedent_relation_builder.json"
    state = MODULE_DIR / ("1983_precedent_relation_builder_state_" + ts + ".json")
    report = REPORTS / ("1983_precedent_relation_builder_raporu_" + ts + ".txt")
