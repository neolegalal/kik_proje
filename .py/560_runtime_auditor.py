...
RUNTIME_DIR = STATE / "production_factory_runtime" / "560_runtime_auditor"
MODULE_ID = "560"
MODULE_NAME = "Runtime Auditor"
...
def analyze(payload):
    ...
    decision='RUNTIME AUDITOR READY' if not validation.get('errors') else 'RUNTIME AUDITOR REVIEW'
...
def main():
    ...
    ts=now_stamp(); output=RUNTIME_DIR/('560_runtime_auditor.json'); state=RUNTIME_DIR/('560_runtime_auditor_state_'+ts+'.json'); report=REPORTS/('560_runtime_auditor_raporu_'+ts+'.txt')
