...
MODULE_DIR = STATE / "unified_decision_processor" / "1957_rag_context_exporter"
MODULE_ID = "1957"
MODULE_NAME = "Rag Context Exporter"
def now_stamp():
...
def main():
    ...
    decision = "RAG CONTEXT EXPORTER READY" if not val["errors"] else "RAG CONTEXT EXPORTER BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1957_rag_context_exporter.json"
    state = MODULE_DIR / ("1957_rag_context_exporter_state_" + ts + ".json")
    report = REPORTS / ("1957_rag_context_exporter_raporu_" + ts + ".txt")
