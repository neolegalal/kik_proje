...
MODULE_DIR = STATE / "production_master" / "907_quality_monitor"
MODULE_ID = "907"
MODULE_NAME = "Quality Monitor"
...
def main():
    ...
    score=val['score']; decision='QUALITY MONITOR READY' if not val['errors'] else 'QUALITY MONITOR BLOCKED'
    ...
    ts=now_stamp(); output=MODULE_DIR/('907_quality_monitor.json'); state=MODULE_DIR/('907_quality_monitor_state_'+ts+'.json'); report=REPORTS/('907_quality_monitor_raporu_'+ts+'.txt')
