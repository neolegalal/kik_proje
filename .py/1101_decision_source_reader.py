...
STATE = BASE / "production_state"; REPORTS = BASE / "raporlar"; MODULE_DIR = STATE / "decision_processing_pipeline" / "1101_decision_source_reader"
MODULE_ID = "1101"; MODULE_NAME = "Decision Source Reader"
...
def main():
    ...
    decision = "DECISION SOURCE READER READY" if not val["errors"] else "DECISION SOURCE READER BLOCKED"
    ...
    ts = now_stamp(); output = MODULE_DIR / "1101_decision_source_reader.json"; state = MODULE_DIR / ("1101_decision_source_reader_state_" + ts + ".json"); report = REPORTS / ("1101_decision_source_reader_raporu_" + ts + ".txt")
