...
IPE_DIR = STATE / "intelligent_production_engine" / "620_ai_cost_token_optimizer"
MODULE_ID = "620"
MODULE_NAME = "AI Cost Token Optimizer"
...
def analyze(payload):
    ...
    decision='AI COST TOKEN OPTIMIZER READY' if not validation.get('errors') else 'AI COST TOKEN OPTIMIZER REVIEW'
...
def main():
    ...
    ts=now_stamp(); output=IPE_DIR/('620_ai_cost_token_optimizer.json'); state=IPE_DIR/('620_ai_cost_token_optimizer_state_'+ts+'.json'); report=REPORTS/('620_ai_cost_token_optimizer_raporu_'+ts+'.txt')
