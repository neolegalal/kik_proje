...
MODULE_DIR = STATE / "neolegal_expert_orchestrator" / "1610_neolegal_expert_certificate"
MODULE_ID = "1610"
MODULE_NAME = "NeoLegal Expert Certificate"
def now_stamp():
...
def main():
    ...
    decision = "NEOLEGAL EXPERT CERTIFICATE READY" if not val["errors"] else "NEOLEGAL EXPERT CERTIFICATE BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1610_neolegal_expert_certificate.json"
    state = MODULE_DIR / ("1610_neolegal_expert_certificate_state_" + ts + ".json")
    report = REPORTS / ("1610_neolegal_expert_certificate_raporu_" + ts + ".txt")
