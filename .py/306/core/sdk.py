...
class NeolegalEnterpriseServicesSDK:
    def __init__(self, name="306.0 NeoLegal Enterprise Services SDK"):
        self.name = name
    ...
    def validate(self, context):
        ...
        decision="NEOLEGAL ENTERPRISE SERVICES CONTEXT READY" if not errors else "NEOLEGAL ENTERPRISE SERVICES CONTEXT BLOCKED"
        return {'score':score,'decision':decision,'errors':errors,'warnings':warnings}

    def plan(self, context, validation):
        if validation['errors']: return {'mode':'PAUSED','operations':[],'message':'NeoLegal Enterprise Services blocked by validation errors.'}
        operations=[]
        ...
        operations.append({'operation':'NEOLEGAL_ENTERPRISE_SERVICES_AUDIT_LOG','status':'PLANNED'})
        return {'mode':'CONTROLLED_NEOLEGAL_ENTERPRISE_SERVICES','operations':operations,'message':str(len(operations))+' neolegal enterprise services operation planned.'}

    def export(self, payload, name=None):
        name=name or '306_0_neolegal_enterprise_services_sdk'
        ensure_dirs(NEOLEGAL_ENTERPRISE_SERVICES_DIR, REPORT_DIR, STATE_DIR)
        ts=now_stamp(); state=STATE_DIR/f'{name}_state_{ts}.json'; report=REPORT_DIR/f'{name}_raporu_{ts}.txt'
        write_json(NEOLEGAL_ENTERPRISE_SERVICES_SNAPSHOT, payload); write_json(state, payload); append_jsonl(NEOLEGAL_ENTERPRISE_SERVICES_HISTORY, payload)
        ...
        write_json(NEOLEGAL_ENTERPRISE_SERVICES_DASHBOARD, dash)
        lines=['='*80,'306.0 NeoLegal Enterprise Services SDK'.upper(),'='*80,'Validation : '+str(payload['validation']['decision']),'Score      : '+str(payload['validation']['score']),'Mode       : '+str(payload['plan']['mode']),'Operations : '+str(len(payload['plan']['operations'])),'','Message:',str(payload['plan']['message']),'','Dosyalar:',str(NEOLEGAL_ENTERPRISE_SERVICES_SNAPSHOT),str(NEOLEGAL_ENTERPRISE_SERVICES_DASHBOARD),str(report)]
        report.write_text('\\n'.join(lines), encoding='utf-8')
        return {'snapshot':str(NEOLEGAL_ENTERPRISE_SERVICES_SNAPSHOT),'dashboard':str(NEOLEGAL_ENTERPRISE_SERVICES_DASHBOARD),'state':str(state),'report':str(report)}
