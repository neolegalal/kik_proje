...
MODULE_DIR = STATE / "neolegal_evolution_platform" / "1908_evidence_collector"
MODULE_ID = "1908"
MODULE_NAME = "Evidence Collector"
def now_stamp():
...
def main():
    ...
    decision = "EVIDENCE COLLECTOR READY" if not val["errors"] else "EVIDENCE COLLECTOR BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1908_evidence_collector.json"
    state = MODULE_DIR / ("1908_evidence_collector_state_" + ts + ".json")
    report = REPORTS / ("1908_evidence_collector_raporu_" + ts + ".txt")
