...
RUNTIME_DIR = STATE / "production_factory_runtime" / "554_parallel_producer"
MODULE_ID = "554"
MODULE_NAME = "Parallel Producer"
...
def analyze(payload):
    ...
    decision='PARALLEL PRODUCER READY' if not validation.get('errors') else 'PARALLEL PRODUCER REVIEW'
...
def main():
    ...
    ts=now_stamp(); output=RUNTIME_DIR/('554_parallel_producer.json'); state=RUNTIME_DIR/('554_parallel_producer_state_'+ts+'.json'); report=REPORTS/('554_parallel_producer_raporu_'+ts+'.txt')
