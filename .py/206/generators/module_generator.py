...
MODULE_DIR = SCHEDULER_DIR / "{safe_id}_{module_slug}"
OUTPUT_FILE = MODULE_DIR / "{safe_id}_{module_slug}.json"

class {class_name}:
    def __init__(self):
        self.sdk = SchedulerSDK(name="{module_id} {module_name}")
...
def create_module(module_id, module_name, bridge_name=None, overwrite=True):
    ...
    ts = now_stamp()
    state = STATE_DIR / ("206_0_module_generator_state_" + ts + ".json")
    report = REPORT_DIR / ("206_0_module_generator_raporu_" + ts + ".txt")
    write_json(state, payload)

    lines = ["="*80, "206.0 MODULE GENERATOR", "="*80, "Score    : " + str(score) + " / 100", "Decision : " + decision, "Target   : " + module_id + " " + module_name, "Errors   : " + str(len(errors)), "Warnings : " + str(len(warnings)), "", "FILES", "-"*80]
...
def main():
    ...
    p = res["payload"]
    print("=" * 80)
    print("206.0 MODULE GENERATOR TAMAMLANDI")
    print("=" * 80)
