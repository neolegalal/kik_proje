...
MODULE_DIR = STATE / "legal_advisory_intelligence" / "1303_legal_basis_finder"
MODULE_ID = "1303"
MODULE_NAME = "Legal Basis Finder"
...
def main():
    ...
    decision = "LEGAL BASIS FINDER READY" if not val["errors"] else "LEGAL BASIS FINDER BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1303_legal_basis_finder.json"
    state = MODULE_DIR / ("1303_legal_basis_finder_state_" + ts + ".json")
    report = REPORTS / ("1303_legal_basis_finder_raporu_" + ts + ".txt")
