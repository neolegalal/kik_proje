# -*- coding: utf-8 -*-
"""
180 - KARAR KARMAŞIKLIK ANALIZ MOTORU

Amaç:
- 168 veya 179 optimize JSONL çıktısını okur.
- Karar metnini dosya yolundan yeniden okur.
- GPT ile kararın doğal bilgi yoğunluğunu ve optimal kart sayısını analiz eder.
- Üretilen kart sayısı optimal aralıkta mı, eksik mi, fazla mı denetler.
- 181 Final Master Production Controller öncesi son planlama kalite katmanıdır.

Kullanım:
  python ".py\\180_Karar_Karmasiklik_Analiz_Motoru.py" "C:\\Users\\MSI\\Desktop\\kik_proje\\uretim_output\\179_optimized_production_output_20260630_212301.jsonl"

Limitli:
  python ".py\\180_Karar_Karmasiklik_Analiz_Motoru.py" "C:\\...\\179_optimized_production_output_x.jsonl" 5

Not:
- API maliyeti oluşur.
- DB'ye yazmaz.
- Büyük üretim öncesi “bu karar için kaç kart yeterlidir?” sorusunu denetler.
"""
...
- İddia sayısı
- Bağımsız hukuki mesele sayısı
- Ayrı mevzuat ekseni sayısı
- Ayrı Kurul değerlendirmesi/sonucu sayısı
- İçtihat/RAG değeri yüksek mesele sayısı
- WEB için anlamlı bağımsız içerik sayısı
- Mevcut kartların bu doğal yoğunluğa uygunluğu
...
Kurallar:
- Eğer actual_card_count optimal aralıkta ve önemli konu eksik değilse PASS.
- Eğer 1-2 kartlık fark veya insan kontrolü gereken konu varsa WARNING.
- Eğer önemli konular eksikse veya kart sayısı belirgin şekilde fazla/azsa FAIL.
""".strip()
...
def normalize_review(obj, karar_no, actual_count):
    ...
    status = str(obj.get("card_count_status", "")).strip().upper()
    if status not in {"ENOUGH", "TOO_FEW", "TOO_MANY", "REVIEW"}:
        mn = as_int("optimal_card_min", actual_count)
        mx = as_int("optimal_card_max", actual_count)
        if actual_count < mn:
            status = "TOO_FEW"
        elif actual_count > mx:
            status = "TOO_MANY"
        else:
            status = "ENOUGH"
    ...
    if decision not in {"PASS", "WARNING", "FAIL"}:
        ...
        if status == "ENOUGH" and overall >= 85 and not missing:
            decision = "PASS"
        elif overall >= 70:
            decision = "WARNING"
        else:
    ...
    return {
        ...
        "optimal_card_min": as_int("optimal_card_min", actual_count),
        "optimal_card_max": as_int("optimal_card_max", actual_count),
        "recommended_card_count": as_int("recommended_card_count", actual_count),
        "actual_card_count": as_int("actual_card_count", actual_count),
        ...
        "overall_planning_score": as_score("overall_planning_score", 0),
        "decision": decision,
...
def main():
    print("=" * 80)
    print("180 - KARAR KARMAŞIKLIK ANALIZ MOTORU")
    print("=" * 80)
    ...
    detail_path = os.path.join(LOG_DIR, f"180_karmasiklik_detay_{run_tag}.jsonl")
    raw_dir = os.path.join(LOG_DIR, f"180_karmasiklik_raw_{run_tag}")
    os.makedirs(raw_dir, exist_ok=True)

    state_path = os.path.join(STATE_DIR, f"180_karmasiklik_state_{run_tag}.json")
    rapor_path = os.path.join(RAPOR_DIR, f"180_karar_karmasiklik_analiz_raporu_{run_tag}.txt")
    ...
    for idx, item in enumerate(items, start=1):
        ...
        try:
            ...
            print(f"{decision} | Plan: {review['overall_planning_score']} | Karmaşıklık: {review['complexity_level']} | Optimal: {review['optimal_card_min']}-{review['optimal_card_max']} | Actual: {review['actual_card_count']} | Token: {usage.get('total_tokens')}")
    ...
    with open(rapor_path, "w", encoding="utf-8") as f:
        f.write("180 - KARAR KARMAŞIKLIK ANALIZ MOTORU RAPORU\n")
        f.write("=" * 80 + "\n\n")
        ...
        for r in all_results:
            ...
            f.write(f"Optimal kart: {rev['optimal_card_min']}-{rev['optimal_card_max']} | Önerilen: {rev['recommended_card_count']} | Mevcut: {rev['actual_card_count']} | Durum: {rev['card_count_status']}\n")
    ...
    print("\n180 KARMAŞIKLIK ANALIZI TAMAMLANDI")
    print("-" * 80)
