...
class CloudPlatformSDK:
    def __init__(self, name="219.0 Cloud Platform SDK"):
        self.name = name
    ...
    def validate(self, context):
        ...
        decision="CLOUD PLATFORM CONTEXT READY" if not errors else "CLOUD PLATFORM CONTEXT BLOCKED"
        return {'score':score,'decision':decision,'errors':errors,'warnings':warnings}

    def plan(self, context, validation):
        if validation['errors']: return {'mode':'PAUSED','operations':[],'message':'Cloud Platform blocked by validation errors.'}
        operations=[]
        ...
        operations.append({'operation':'CLOUD_PLATFORM_AUDIT_LOG','status':'PLANNED'})
        return {'mode':'CONTROLLED_CLOUD_PLATFORM','operations':operations,'message':str(len(operations))+' cloud platform operation planned.'}

    def export(self, payload, name=None):
        name=name or '219_0_cloud_platform_sdk'
        ensure_dirs(CLOUD_PLATFORM_DIR, REPORT_DIR, STATE_DIR)
        ts=now_stamp(); state=STATE_DIR/f'{name}_state_{ts}.json'; report=REPORT_DIR/f'{name}_raporu_{ts}.txt'
        write_json(CLOUD_PLATFORM_SNAPSHOT, payload); write_json(state, payload); append_jsonl(CLOUD_PLATFORM_HISTORY, payload)
        ...
        write_json(CLOUD_PLATFORM_DASHBOARD, dash)
        lines=['='*80,'219.0 Cloud Platform SDK'.upper(),'='*80,'Validation : '+str(payload['validation']['decision']),'Score      : '+str(payload['validation']['score']),'Mode       : '+str(payload['plan']['mode']),'Operations : '+str(len(payload['plan']['operations'])),'','Message:',str(payload['plan']['message']),'','Dosyalar:',str(CLOUD_PLATFORM_SNAPSHOT),str(CLOUD_PLATFORM_DASHBOARD),str(report)]
        report.write_text('\\n'.join(lines), encoding='utf-8')
        return {'snapshot':str(CLOUD_PLATFORM_SNAPSHOT),'dashboard':str(CLOUD_PLATFORM_DASHBOARD),'state':str(state),'report':str(report)}
