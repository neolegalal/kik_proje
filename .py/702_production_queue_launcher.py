...
PILOT_MODULE_DIR = STATE / "pilot_production" / "702_production_queue_launcher"
MODULE_ID = "702"
MODULE_NAME = "Production Queue Launcher"
...
def analyze(payload):
    ...
    decision='PRODUCTION QUEUE LAUNCHER READY' if not validation.get('errors') else 'PRODUCTION QUEUE LAUNCHER REVIEW'
...
def main():
    ...
    ts=now_stamp(); output=PILOT_MODULE_DIR/('702_production_queue_launcher.json'); state=PILOT_MODULE_DIR/('702_production_queue_launcher_state_'+ts+'.json'); report=REPORTS/('702_production_queue_launcher_raporu_'+ts+'.txt')
