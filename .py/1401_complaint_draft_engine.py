...
MODULE_DIR = STATE / "litigation_intelligence" / "1401_complaint_draft_engine"
MODULE_ID = "1401"
MODULE_NAME = "Complaint Draft Engine"
def now_stamp():
...
def main():
    ...
    decision = "COMPLAINT DRAFT ENGINE READY" if not val["errors"] else "COMPLAINT DRAFT ENGINE BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1401_complaint_draft_engine.json"
    state = MODULE_DIR / ("1401_complaint_draft_engine_state_" + ts + ".json")
    report = REPORTS / ("1401_complaint_draft_engine_raporu_" + ts + ".txt")
