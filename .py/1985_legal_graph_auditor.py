...
MODULE_DIR = STATE / "case_graph_trainer" / "1985_legal_graph_auditor"
MODULE_ID = "1985"
MODULE_NAME = "Legal Graph Auditor"
def now_stamp():
...
def main():
    ...
    decision = "LEGAL GRAPH AUDITOR READY" if not val["errors"] else "LEGAL GRAPH AUDITOR BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1985_legal_graph_auditor.json"
    state = MODULE_DIR / ("1985_legal_graph_auditor_state_" + ts + ".json")
    report = REPORTS / ("1985_legal_graph_auditor_raporu_" + ts + ".txt")
