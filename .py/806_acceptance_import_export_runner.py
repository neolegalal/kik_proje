...
MODULE_DIR = STATE / "real_production_engine" / "806_acceptance_import_export_runner"
MODULE_ID = "806"
MODULE_NAME = "Acceptance Import Export Runner"
...
def analyze(payload):
    ...
    decision='ACCEPTANCE IMPORT EXPORT RUNNER READY' if not validation.get('errors') else 'ACCEPTANCE IMPORT EXPORT RUNNER REVIEW'
...
def main():
    ...
    ts=now_stamp(); output=MODULE_DIR/('806_acceptance_import_export_runner.json'); state=MODULE_DIR/('806_acceptance_import_export_runner_state_'+ts+'.json'); report=REPORTS/('806_acceptance_import_export_runner_raporu_'+ts+'.txt')
