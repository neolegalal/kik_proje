...
FACTORY_DIR = STATE / "production_data_factory" / "510_large_scale_launch_manager"
MODULE_ID = "510"
MODULE_NAME = "Large Scale Launch Manager"
...
def analyze(payload):
    ...
    decision='LARGE SCALE LAUNCH MANAGER READY' if not validation.get('errors') else 'LARGE SCALE LAUNCH MANAGER REVIEW'
...
def main():
    ...
    ts=now_stamp(); output=FACTORY_DIR/('510_large_scale_launch_manager.json'); state=FACTORY_DIR/('510_large_scale_launch_manager_state_'+ts+'.json'); report=REPORTS/('510_large_scale_launch_manager_raporu_'+ts+'.txt')
