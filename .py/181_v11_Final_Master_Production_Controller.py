# -*- coding: utf-8 -*-
"""
181 v11 - FINAL MASTER PRODUCTION CONTROLLER / STABLE ORCHESTRATOR
...
def step_ok_182():
    ...
    decision = (s.get("final_decision") or s.get("decision") or "").upper()
    new_score = as_float(s.get("new_model_drift_score", s.get("drift_score", 0)))
    pool_score = as_float(s.get("overall_pool_drift_score", s.get("general_drift_score", 0)))
    ok = decision == "PASS" or new_score >= 85 or pool_score >= 85
    return ok, f"decision={decision} new_score={new_score} pool_score={pool_score}"
...
def compute_final_ready(steps, overall_ok, recovered_steps):
    ...
    for st in steps:
        if st.get("skipped") or st.get("non_blocking"):
            continue
        if st.get("step") in recovered_steps:
            continue
...
def main():
    print("=" * 80)
    print("181 v11 - FINAL MASTER PRODUCTION CONTROLLER / STABLE ORCHESTRATOR")
    print("=" * 80)

    run_tag = tag()
    limit, flags = parse_args()

    log_path = os.path.join(LOG_DIR, f"181_v11_final_master_controller_log_{run_tag}.txt")
    state_path = os.path.join(STATE_DIR, f"181_v11_final_master_controller_state_{run_tag}.json")
    rapor_path = os.path.join(RAPOR_DIR, f"181_v11_final_master_controller_raporu_{run_tag}.txt")
    ...
    # v11 recovery-aware final decision:
    # 172/177 ilk deneme FAIL ise ama self-healing sonrası PASS ise blok sayılmaz.
    ...
    # Dashboard ve supervisor 181 state dosyasını okuduğu için,
    # 184/190 çalışmadan ÖNCE güncel 181 state snapshot yazılır.
    ...
    if overall_ok and not flags["skip_182"]:
        ...
        overall_ok = record_step(steps, "182", res, step_ok_182) and overall_ok
    elif flags["skip_182"]:
    ...
    if overall_ok and not flags["skip_183"]:
        ...
        overall_ok = record_step(steps, "183", res, step_ok_183) and overall_ok
    elif flags["skip_183"]:
        record_step(steps, "183", {}, skipped=True)

    # 182/183 sonrası 184 yine güncel 181 state'i okuyabilsin diye snapshot yenilenir.
    recovered_steps = compute_recovered_steps(ai_cleaning_summary, healing_summary)
    final_ready, hard_steps = compute_final_ready(steps, overall_ok, recovered_steps)
    state = write_state_snapshot(
        state_path, run_tag, limit, flags, output_168, current_output,
    ...
    if overall_ok and not flags["skip_184"]:
        ...
        # Dashboard görünüm katmanıdır; state uyumsuzluğu batch'i durdurmasın.
        record_step(steps, "184", res, step_ok_184, non_blocking=True)
    elif flags["skip_184"]:
        record_step(steps, "184", {}, skipped=True)

    # Supervisor da 181 state okur; 190 öncesi yeniden snapshot.
    recovered_steps = compute_recovered_steps(ai_cleaning_summary, healing_summary)
    final_ready, hard_steps = compute_final_ready(steps, overall_ok, recovered_steps)
    state = write_state_snapshot(
        state_path, run_tag, limit, flags, output_168, current_output,
    ...
    if overall_ok and not flags["skip_190"]:
        ...
        # Supervisor kontrol/öneri katmanıdır; eski legacy uyarısı batch'i durdurmasın.
        record_step(steps, "190", res, step_ok_190, non_blocking=True)
    elif flags["skip_190"]:
        record_step(steps, "190", {}, skipped=True)

    # Son state: 184/190 dahil tüm adımlar raporda görünsün.
    recovered_steps = compute_recovered_steps(ai_cleaning_summary, healing_summary)
    final_ready, hard_steps = compute_final_ready(steps, overall_ok, recovered_steps)
    state = write_state_snapshot(
        state_path, run_tag, limit, flags, output_168, current_output,
    ...
    with open(rapor_path, "w", encoding="utf-8") as f:
        f.write("181 v11 - FINAL MASTER PRODUCTION CONTROLLER / STABLE ORCHESTRATOR RAPORU\n")
        f.write("=" * 80 + "\n\n")
    ...
    print("\n181 v11 FINAL MASTER CONTROLLER TAMAMLANDI")
    print("-" * 80)
