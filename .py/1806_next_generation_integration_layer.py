...
MODULE_DIR = STATE / "next_generation_neolegal" / "1806_next_generation_integration_layer"
MODULE_ID = "1806"
MODULE_NAME = "Next Generation Integration Layer"
def now_stamp():
...
def main():
    ...
    decision = "NEXT GENERATION INTEGRATION LAYER READY" if not val["errors"] else "NEXT GENERATION INTEGRATION LAYER BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1806_next_generation_integration_layer.json"
    state = MODULE_DIR / ("1806_next_generation_integration_layer_state_" + ts + ".json")
    report = REPORTS / ("1806_next_generation_integration_layer_raporu_" + ts + ".txt")
