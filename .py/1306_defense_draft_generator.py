...
MODULE_DIR = STATE / "legal_advisory_intelligence" / "1306_defense_draft_generator"
MODULE_ID = "1306"
MODULE_NAME = "Defense Draft Generator"
...
def main():
    ...
    decision = "DEFENSE DRAFT GENERATOR READY" if not val["errors"] else "DEFENSE DRAFT GENERATOR BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1306_defense_draft_generator.json"
    state = MODULE_DIR / ("1306_defense_draft_generator_state_" + ts + ".json")
    report = REPORTS / ("1306_defense_draft_generator_raporu_" + ts + ".txt")
