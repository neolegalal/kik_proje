...
MODULE_DIR = STATE / "client_workspace_memory" / "1704_document_memory_indexer"
MODULE_ID = "1704"
MODULE_NAME = "Document Memory Indexer"
def now_stamp():
...
def main():
    ...
    decision = "DOCUMENT MEMORY INDEXER READY" if not val["errors"] else "DOCUMENT MEMORY INDEXER BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1704_document_memory_indexer.json"
    state = MODULE_DIR / ("1704_document_memory_indexer_state_" + ts + ".json")
    report = REPORTS / ("1704_document_memory_indexer_raporu_" + ts + ".txt")
