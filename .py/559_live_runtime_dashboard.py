...
RUNTIME_DIR = STATE / "production_factory_runtime" / "559_live_runtime_dashboard"
MODULE_ID = "559"
MODULE_NAME = "Live Runtime Dashboard"
...
def analyze(payload):
    ...
    decision='LIVE RUNTIME DASHBOARD READY' if not validation.get('errors') else 'LIVE RUNTIME DASHBOARD REVIEW'
...
def main():
    ...
    ts=now_stamp(); output=RUNTIME_DIR/('559_live_runtime_dashboard.json'); state=RUNTIME_DIR/('559_live_runtime_dashboard_state_'+ts+'.json'); report=REPORTS/('559_live_runtime_dashboard_raporu_'+ts+'.txt')
