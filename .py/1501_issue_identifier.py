...
MODULE_DIR = STATE / "legal_reasoning_engine" / "1501_issue_identifier"
MODULE_ID = "1501"
MODULE_NAME = "Issue Identifier"
def now_stamp():
...
def main():
    ...
    decision = "ISSUE IDENTIFIER READY" if not val["errors"] else "ISSUE IDENTIFIER BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1501_issue_identifier.json"
    state = MODULE_DIR / ("1501_issue_identifier_state_" + ts + ".json")
    report = REPORTS / ("1501_issue_identifier_raporu_" + ts + ".txt")
