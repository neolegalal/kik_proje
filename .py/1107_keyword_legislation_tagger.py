...
STATE = BASE / "production_state"; REPORTS = BASE / "raporlar"; MODULE_DIR = STATE / "decision_processing_pipeline" / "1107_keyword_legislation_tagger"
MODULE_ID = "1107"; MODULE_NAME = "Keyword Legislation Tagger"
...
def main():
    ...
    decision = "KEYWORD LEGISLATION TAGGER READY" if not val["errors"] else "KEYWORD LEGISLATION TAGGER BLOCKED"
    ...
    ts = now_stamp(); output = MODULE_DIR / "1107_keyword_legislation_tagger.json"; state = MODULE_DIR / ("1107_keyword_legislation_tagger_state_" + ts + ".json"); report = REPORTS / ("1107_keyword_legislation_tagger_raporu_" + ts + ".txt")
