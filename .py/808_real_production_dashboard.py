...
MODULE_DIR = STATE / "real_production_engine" / "808_real_production_dashboard"
MODULE_ID = "808"
MODULE_NAME = "Real Production Dashboard"
...
def analyze(payload):
    ...
    decision='REAL PRODUCTION DASHBOARD READY' if not validation.get('errors') else 'REAL PRODUCTION DASHBOARD REVIEW'
...
def main():
    ...
    ts=now_stamp(); output=MODULE_DIR/('808_real_production_dashboard.json'); state=MODULE_DIR/('808_real_production_dashboard_state_'+ts+'.json'); report=REPORTS/('808_real_production_dashboard_raporu_'+ts+'.txt')
