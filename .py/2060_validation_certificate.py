...
MODULE_DIR = STATE / "validation_benchmark_platform" / "2060_validation_certificate"
MODULE_ID = "2060"
MODULE_NAME = "Validation Certificate"
def now_stamp():
...
def main():
    ...
    decision = "VALIDATION CERTIFICATE READY" if not val["errors"] else "VALIDATION CERTIFICATE BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "2060_validation_certificate.json"
    state = MODULE_DIR / ("2060_validation_certificate_state_" + ts + ".json")
    report = REPORTS / ("2060_validation_certificate_raporu_" + ts + ".txt")
