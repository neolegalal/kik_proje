...
MODULE_DIR = STATE / "legal_advisory_intelligence" / "1310_advisory_quality_auditor"
MODULE_ID = "1310"
MODULE_NAME = "Advisory Quality Auditor"
...
def main():
    ...
    decision = "ADVISORY QUALITY AUDITOR READY" if not val["errors"] else "ADVISORY QUALITY AUDITOR BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1310_advisory_quality_auditor.json"
    state = MODULE_DIR / ("1310_advisory_quality_auditor_state_" + ts + ".json")
    report = REPORTS / ("1310_advisory_quality_auditor_raporu_" + ts + ".txt")
