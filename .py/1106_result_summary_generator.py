...
STATE = BASE / "production_state"; REPORTS = BASE / "raporlar"; MODULE_DIR = STATE / "decision_processing_pipeline" / "1106_result_summary_generator"
MODULE_ID = "1106"; MODULE_NAME = "Result Summary Generator"
...
def main():
    ...
    decision = "RESULT SUMMARY GENERATOR READY" if not val["errors"] else "RESULT SUMMARY GENERATOR BLOCKED"
    ...
    ts = now_stamp(); output = MODULE_DIR / "1106_result_summary_generator.json"; state = MODULE_DIR / ("1106_result_summary_generator_state_" + ts + ".json"); report = REPORTS / ("1106_result_summary_generator_raporu_" + ts + ".txt")
