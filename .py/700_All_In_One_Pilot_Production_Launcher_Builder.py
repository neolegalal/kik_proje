...
def sdk_source():
    ...
    x("        rows=[]")
    x("        for file in PIPELINE_FILES:")
    x("            p=PY/file")
    x("            rows.append({'file':file,'path':str(p),'exists':p.exists()})")
    x("        return rows")
    ...
    x("        errors=[]; warnings=[]")
    x("        if pipe_score < 50: errors.append('Ana production pipeline dosyalarının çoğu bulunamadı.')")
