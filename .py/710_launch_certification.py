...
PILOT_MODULE_DIR = STATE / "pilot_production" / "710_launch_certification"
MODULE_ID = "710"
MODULE_NAME = "Launch Certification"
...
def analyze(payload):
    ...
    decision='LAUNCH CERTIFICATION READY' if not validation.get('errors') else 'LAUNCH CERTIFICATION REVIEW'
...
def main():
    ...
    ts=now_stamp(); output=PILOT_MODULE_DIR/('710_launch_certification.json'); state=PILOT_MODULE_DIR/('710_launch_certification_state_'+ts+'.json'); report=REPORTS/('710_launch_certification_raporu_'+ts+'.txt')
