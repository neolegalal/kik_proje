...
MODULE_DIR = STATE / "legal_advisory_intelligence" / "1302_claim_defense_mapper"
MODULE_ID = "1302"
MODULE_NAME = "Claim Defense Mapper"
...
def main():
    ...
    decision = "CLAIM DEFENSE MAPPER READY" if not val["errors"] else "CLAIM DEFENSE MAPPER BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1302_claim_defense_mapper.json"
    state = MODULE_DIR / ("1302_claim_defense_mapper_state_" + ts + ".json")
    report = REPORTS / ("1302_claim_defense_mapper_raporu_" + ts + ".txt")
