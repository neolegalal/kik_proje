...
MODULE_DIR = STATE / "unified_decision_processor" / "1958_unified_quality_gate"
MODULE_ID = "1958"
MODULE_NAME = "Unified Quality Gate"
def now_stamp():
...
def main():
    ...
    decision = "UNIFIED QUALITY GATE READY" if not val["errors"] else "UNIFIED QUALITY GATE BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1958_unified_quality_gate.json"
    state = MODULE_DIR / ("1958_unified_quality_gate_state_" + ts + ".json")
    report = REPORTS / ("1958_unified_quality_gate_raporu_" + ts + ".txt")
