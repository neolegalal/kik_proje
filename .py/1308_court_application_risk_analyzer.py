...
MODULE_DIR = STATE / "legal_advisory_intelligence" / "1308_court_application_risk_analyzer"
MODULE_ID = "1308"
MODULE_NAME = "Court Application Risk Analyzer"
...
def main():
    ...
    decision = "COURT APPLICATION RISK ANALYZER READY" if not val["errors"] else "COURT APPLICATION RISK ANALYZER BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1308_court_application_risk_analyzer.json"
    state = MODULE_DIR / ("1308_court_application_risk_analyzer_state_" + ts + ".json")
    report = REPORTS / ("1308_court_application_risk_analyzer_raporu_" + ts + ".txt")
