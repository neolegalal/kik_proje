...
MODULE_DIR = STATE / "case_graph_trainer" / "1973_sub_issue_mapper"
MODULE_ID = "1973"
MODULE_NAME = "Sub Issue Mapper"
def now_stamp():
...
def main():
    ...
    decision = "SUB ISSUE MAPPER READY" if not val["errors"] else "SUB ISSUE MAPPER BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1973_sub_issue_mapper.json"
    state = MODULE_DIR / ("1973_sub_issue_mapper_state_" + ts + ".json")
    report = REPORTS / ("1973_sub_issue_mapper_raporu_" + ts + ".txt")
