...
RUNTIME_DIR = STATE / "production_factory_runtime" / "556_cost_optimizer"
MODULE_ID = "556"
MODULE_NAME = "Cost Optimizer"
...
def analyze(payload):
    ...
    decision='COST OPTIMIZER READY' if not validation.get('errors') else 'COST OPTIMIZER REVIEW'
...
def main():
    ...
    ts=now_stamp(); output=RUNTIME_DIR/('556_cost_optimizer.json'); state=RUNTIME_DIR/('556_cost_optimizer_state_'+ts+'.json'); report=REPORTS/('556_cost_optimizer_raporu_'+ts+'.txt')
