...
RUNTIME_DIR = STATE / "production_factory_runtime" / "558_batch_executor_runtime"
MODULE_ID = "558"
MODULE_NAME = "Batch Executor Runtime"
...
def analyze(payload):
    ...
    decision='BATCH EXECUTOR RUNTIME READY' if not validation.get('errors') else 'BATCH EXECUTOR RUNTIME REVIEW'
...
def main():
    ...
    ts=now_stamp(); output=RUNTIME_DIR/('558_batch_executor_runtime.json'); state=RUNTIME_DIR/('558_batch_executor_runtime_state_'+ts+'.json'); report=REPORTS/('558_batch_executor_runtime_raporu_'+ts+'.txt')
