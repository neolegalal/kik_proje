...
STATE = BASE / "production_state"; REPORTS = BASE / "raporlar"; MODULE_DIR = STATE / "decision_processing_pipeline" / "1105_decision_summary_generator"
MODULE_ID = "1105"; MODULE_NAME = "Decision Summary Generator"
...
def main():
    ...
    decision = "DECISION SUMMARY GENERATOR READY" if not val["errors"] else "DECISION SUMMARY GENERATOR BLOCKED"
    ...
    ts = now_stamp(); output = MODULE_DIR / "1105_decision_summary_generator.json"; state = MODULE_DIR / ("1105_decision_summary_generator_state_" + ts + ".json"); report = REPORTS / ("1105_decision_summary_generator_raporu_" + ts + ".txt")
