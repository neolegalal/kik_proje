...
MODULE_DIR = STATE / "production_safety_gate" / "818_resource_capacity_checker"
MODULE_ID = "818"
MODULE_NAME = "Resource Capacity Checker"
...
def main():
    ...
    decision='RESOURCE CAPACITY CHECKER READY' if final['decision'] in ('PASS','WARN') else 'RESOURCE CAPACITY CHECKER BLOCKED'
    ...
    ts=now_stamp(); output=MODULE_DIR/('818_resource_capacity_checker.json'); state=MODULE_DIR/('818_resource_capacity_checker_state_'+ts+'.json'); report=REPORTS/('818_resource_capacity_checker_raporu_'+ts+'.txt')
