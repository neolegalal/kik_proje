# -*- coding: utf-8 -*-
"""
181 v6 - FINAL MASTER PRODUCTION CONTROLLER / SELF-HEALING
...
def main():
    print("=" * 80)
    print("181 v6 - FINAL MASTER PRODUCTION CONTROLLER / SELF-HEALING")
    print("=" * 80)

    run_tag = tag()
    limit, flags = parse_args()

    log_path = os.path.join(LOG_DIR, f"181_v6_final_master_controller_log_{run_tag}.txt")
    state_path = os.path.join(STATE_DIR, f"181_v6_final_master_controller_state_{run_tag}.json")
    rapor_path = os.path.join(RAPOR_DIR, f"181_v6_final_master_controller_raporu_{run_tag}.txt")
    ...
    if overall_ok:
        ...
        res = run_cmd(["python", SCRIPTS["171"], current_output], log_path)
        overall_ok = record_step(steps, "171", res, step_ok_171) and overall_ok

    if overall_ok and not flags["skip_172"]:
        print("\n[3] 172 AI kalite")
        res = run_cmd(["python", SCRIPTS["172"], current_output], log_path)
    ...
    with open(rapor_path, "w", encoding="utf-8") as f:
        f.write("181 v6 - FINAL MASTER PRODUCTION CONTROLLER / SELF-HEALING RAPORU\n")
        f.write("=" * 80 + "\n\n")
    ...
    print("\n181 v6 FINAL MASTER CONTROLLER TAMAMLANDI")
    print("-" * 80)
    ...
    if healing_summary:
        print(f"Self-healing durumu           : {healing_summary.get('final_status')}")
