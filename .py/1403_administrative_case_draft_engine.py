...
MODULE_DIR = STATE / "litigation_intelligence" / "1403_administrative_case_draft_engine"
MODULE_ID = "1403"
MODULE_NAME = "Administrative Case Draft Engine"
def now_stamp():
...
def main():
    ...
    decision = "ADMINISTRATIVE CASE DRAFT ENGINE READY" if not val["errors"] else "ADMINISTRATIVE CASE DRAFT ENGINE BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1403_administrative_case_draft_engine.json"
    state = MODULE_DIR / ("1403_administrative_case_draft_engine_state_" + ts + ".json")
    report = REPORTS / ("1403_administrative_case_draft_engine_raporu_" + ts + ".txt")
