...
OS_DIR = STATE / "production_os" / "405_production_dashboard"
MODULE_ID = "405"
MODULE_NAME = "Production Dashboard"
...
def analyze(payload):
    ...
    decision = 'PRODUCTION DASHBOARD READY' if not validation.get('errors') else 'PRODUCTION DASHBOARD REVIEW'
...
def main():
    ...
    ts = now_stamp()
    output = OS_DIR / ('405_production_dashboard.json')
    state = OS_DIR / ('405_production_dashboard_state_' + ts + '.json')
    report = REPORTS / ('405_production_dashboard_raporu_' + ts + '.txt')
