...
PILOT_MODULE_DIR = STATE / "pilot_production" / "704_live_quality_monitor"
MODULE_ID = "704"
MODULE_NAME = "Live Quality Monitor"
...
def analyze(payload):
    ...
    decision='LIVE QUALITY MONITOR READY' if not validation.get('errors') else 'LIVE QUALITY MONITOR REVIEW'
...
def main():
    ...
    ts=now_stamp(); output=PILOT_MODULE_DIR/('704_live_quality_monitor.json'); state=PILOT_MODULE_DIR/('704_live_quality_monitor_state_'+ts+'.json'); report=REPORTS/('704_live_quality_monitor_raporu_'+ts+'.txt')
