...
STATE = BASE / "production_state"; REPORTS = BASE / "raporlar"; MODULE_DIR = STATE / "decision_processing_pipeline" / "1103_decision_metadata_extractor"
MODULE_ID = "1103"; MODULE_NAME = "Decision Metadata Extractor"
...
def main():
    ...
    decision = "DECISION METADATA EXTRACTOR READY" if not val["errors"] else "DECISION METADATA EXTRACTOR BLOCKED"
    ...
    ts = now_stamp(); output = MODULE_DIR / "1103_decision_metadata_extractor.json"; state = MODULE_DIR / ("1103_decision_metadata_extractor_state_" + ts + ".json"); report = REPORTS / ("1103_decision_metadata_extractor_raporu_" + ts + ".txt")
