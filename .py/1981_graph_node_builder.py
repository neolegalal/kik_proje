...
MODULE_DIR = STATE / "case_graph_trainer" / "1981_graph_node_builder"
MODULE_ID = "1981"
MODULE_NAME = "Graph Node Builder"
def now_stamp():
...
def main():
    ...
    decision = "GRAPH NODE BUILDER READY" if not val["errors"] else "GRAPH NODE BUILDER BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1981_graph_node_builder.json"
    state = MODULE_DIR / ("1981_graph_node_builder_state_" + ts + ".json")
    report = REPORTS / ("1981_graph_node_builder_raporu_" + ts + ".txt")
