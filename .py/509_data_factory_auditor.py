...
FACTORY_DIR = STATE / "production_data_factory" / "509_data_factory_auditor"
MODULE_ID = "509"
MODULE_NAME = "Data Factory Auditor"
...
def analyze(payload):
    ...
    decision='DATA FACTORY AUDITOR READY' if not validation.get('errors') else 'DATA FACTORY AUDITOR REVIEW'
...
def main():
    ...
    ts=now_stamp(); output=FACTORY_DIR/('509_data_factory_auditor.json'); state=FACTORY_DIR/('509_data_factory_auditor_state_'+ts+'.json'); report=REPORTS/('509_data_factory_auditor_raporu_'+ts+'.txt')
