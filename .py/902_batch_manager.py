...
MODULE_DIR = STATE / "production_master" / "902_batch_manager"
MODULE_ID = "902"
MODULE_NAME = "Batch Manager"
...
def main():
    ...
    score=val['score']; decision='BATCH MANAGER READY' if not val['errors'] else 'BATCH MANAGER BLOCKED'
    ...
    ts=now_stamp(); output=MODULE_DIR/('902_batch_manager.json'); state=MODULE_DIR/('902_batch_manager_state_'+ts+'.json'); report=REPORTS/('902_batch_manager_raporu_'+ts+'.txt')
