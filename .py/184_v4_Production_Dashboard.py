...
Kullanım:
  python ".py\\184_v2_Production_Dashboard.py"
"""
...
def metric_html(label, value, sub="", status="neutral"):
    return f"""
<div class="metric {escape(status)}">
...
def table_rows(rows):
    html = "<table><tr><th>Alan</th><th>Değer</th><th>Not</th></tr>"
    for name, value, note in rows:
        html += (
            f"<tr><td>{escape(str(name))}</td>"
            f"<td>{escape(str(value))}</td>"
            f"<td>{escape(str(note))}</td></tr>"
        )
    html += "</table>"
    return html
...
def build_dashboard():
    ...
    prefixes = {
        "181": "181_v8_final_master_controller_state_",
    ...
    for key, prefix in prefixes.items():
        path, state = latest_state(prefix)
        states[key] = {"path": path, "state": state or {}}

    # 181 v8 yoksa eski sürümlere sırayla düş.
    if not states["181"]["state"]:
        for old_prefix in [
            "181_v7_final_master_controller_state_",
            "181_v6_final_master_controller_state_",
            "181_v5_final_master_controller_state_",
            "181_v4_final_master_controller_state_",
            "181_v3_final_master_controller_state_",
        ]:
            path, state = latest_state(old_prefix)
            if state:
                states["181"] = {"path": path, "state": state or {}}
                break

    db = db_counts()

    s181 = states["181"]["state"]
    s182 = states["182"]["state"]
    s183 = states["183"]["state"]
    s170 = states["170"]["state"]
    s177 = states["177"]["state"]
    s175 = states["175"]["state"]
    s176 = states["176"]["state"]
    s178 = states["178"]["state"]
    s179 = states["179"]["state"]
    s180 = states["180"]["state"]
    s173 = states["173"]["state"]
    ...
    sampling_ready = bool(s183.get("ready_for_ai_qa", False))
    sampling_partial = bool(s183.get("ready_for_184", False))
    sample_count = s183.get("sample_count_created", "YOK")

    warnings = []

    if drift_decision == "WARNING":
        warnings.append("Drift analizi WARNING verdi. Bu genellikle yeni model örnek sayısı henüz düşükken normaldir.")
    elif drift_decision == "FAIL":
        warnings.append("Drift analizi FAIL verdi. Üretim öncesi rapor incelenmeli.")

    if not sampling_ready and sampling_partial:
        warnings.append("Sampling QA kısmen hazır. Bazı örneklerin production row'u bulunamadı.")
    elif not sampling_ready:
        warnings.append("Sampling QA hazır değil.")

    if s180 and as_float(s180.get("avg_planning_score", 0)) < 70:
        warnings.append("180 karmaşıklık planlama puanı düşük; eksik kart/fail yoksa bloklayıcı değildir.")

    # v3:
    # 181 v4 nihai üretim kararını esas alır.
    # 173 ve 183 bazı durumlarda non-blocking uyarı verebilir.
    overall_ok = final_ready and drift_decision in {"PASS", "WARNING"}

    html_path = os.path.join(DASHBOARD_DIR, f"184_v4_production_dashboard_{run_tag}.html")
    txt_path = os.path.join(RAPOR_DIR, f"184_v4_production_dashboard_raporu_{run_tag}.txt")
    state_path = os.path.join(STATE_DIR, f"184_v4_production_dashboard_state_{run_tag}.json")

    metrics = ""
    metrics += metric_html("Final Üretim Durumu", "EVET" if final_ready else "HAYIR", "181 v8 sonucu", "ok" if final_ready else "bad")
    metrics += metric_html("Aktif Export Kart", active_export, "170 Web/RAG export", "ok" if as_int(active_export) > 0 else "bad")
    metrics += metric_html(
        "Drift Skoru",
        f"{drift_score}/100",
        f"Karar: {drift_decision}",
        "ok" if drift_decision == "PASS" else "warn" if drift_decision == "WARNING" else "bad"
    )
    metrics += metric_html(
        "Yeni Model Kart",
        new_model_cards,
        f"Eski model: {legacy_cards}",
        "ok" if as_int(new_model_cards) >= 100 else "warn" if as_int(new_model_cards) > 0 else "bad"
    )
    metrics += metric_html(
        "Sampling QA",
        sample_count,
        "Hazır: EVET" if sampling_ready else "Hazır: KISMEN" if sampling_partial else "Hazır: HAYIR",
        "ok" if sampling_ready else "warn" if sampling_partial else "bad"
    )
    metrics += metric_html("DB Aktif Kart", db.get("active_cards", "YOK"), "SQLite", "ok" if db.get("active_cards", 0) > 0 else "bad")
    ...
    quality_rows = [
        ...
        ("176 Priority", s176.get("avg_priority_coverage", "YOK"), f"ready={s176.get('ready_for_177')}"),
        ("177 Hukuki Doğruluk", s177.get("avg_legal_accuracy_score", "YOK"), f"fail_cards={s177.get('fail_cards')}"),
        ("178 Merge", s178.get("reduction_rate", "YOK"), f"merge={s178.get('merge_recommended_count')}"),
        ("179 Optimizasyon", s179.get("reduction_rate", "YOK"), f"ready={s179.get('ready_for_180')}"),
        ("180 Karmaşıklık", s180.get("avg_planning_score", "YOK"), f"too_few={s180.get('too_few_count')}"),
        ("173 Acceptance", s173.get("score", "YOK"), f"ready={s173.get('master_ready_for_large_production')}"),
        ("182 Drift", drift_score, f"decision={drift_decision}"),
        ("183 Sampling", sample_count, f"ready_ai={sampling_ready}"),
    ]
...
<head>
<meta charset="utf-8">
<title>184 v4 Production Dashboard</title>
<style>
...
.badge{{display:inline-block;border-radius:999px;padding:7px 12px;margin-top:8px;background:#F4C542;color:#0E1A33;font-weight:bold}}
...
.metric.neutral{{border-top-color:#94a3b8}}
.label{{color:#64748b;font-size:13px;font-weight:bold}}
.value{{font-size:28px;font-weight:bold;margin:8px 0;color:#0E1A33}}
.sub{{font-size:13px;color:#64748b}}
.card{{background:white;border-radius:16px;padding:20px;margin-bottom:18px;box-shadow:0 2px 10px rgba(0,0,0,.06)}}
h2{{margin-top:0;color:#0E1A33}}
table{{width:100%;border-collapse:collapse}}
td,th{{border-bottom:1px solid #e5e7eb;text-align:left;padding:9px;vertical-align:top}}
...
<body>
<div class="header">
  <h1>184 v4 Production Dashboard</h1>
...
{section("State Dosyaları", table_rows(state_rows))}
    ...
    with open(txt_path, "w", encoding="utf-8") as f:
        f.write("184 v4 - PRODUCTION DASHBOARD RAPORU\n")
        f.write("=" * 80 + "\n\n")
        ...
        f.write(f"Sampling QA                   : {'EVET' if sampling_ready else 'KISMEN' if sampling_partial else 'HAYIR'}\n\n")
    ...
    dashboard_state = {
        ...
        "sampling_partial": sampling_partial,
        "warnings": warnings,
        "html_path": html_path,
        "txt_path": txt_path,
        "ready_for_185": True,
...
def main():
    print("=" * 80)
    print("184 v4 - PRODUCTION DASHBOARD")
    print("=" * 80)

    html_path, txt_path, state_path, st = build_dashboard()

    print("\n184 v4 PRODUCTION DASHBOARD TAMAMLANDI")
    print("-" * 80)
    ...
    print(f"Sampling QA                   : {'EVET' if st['sampling_ready'] else 'KISMEN' if st['sampling_partial'] else 'HAYIR'}")
