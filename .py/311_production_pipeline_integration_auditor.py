...
AUDIT_DIR = STATE / "production_readiness" / '311_production_pipeline_integration_auditor'
MODULE_ID = '311'
MODULE_NAME = 'Production Pipeline Integration Auditor'
VERSION = 'v2.18'
TAG = 'v2.18-production-pipeline-integration-auditor'
MUST_HAVE = ['206', '207', '168', '172', '177', '173', '169', '170', '190', '195']
PATTERNS = ['scheduler', 'execution', 'production', 'quality', 'legal', 'acceptance', 'export', 'runtime']
...
def create_docs():
    ...
    release_path = RELEASES / 'v2.18-production-pipeline-integration-auditor.md'
...
def main():
    ...
    ts = now_stamp()
    state_path = AUDIT_DIR / (MODULE_ID + "_production_pipeline_integration_auditor_audit_" + ts + ".json")
    report_path = REPORTS / (MODULE_ID + "_production_pipeline_integration_auditor_raporu_" + ts + ".txt")
