...
class KnowledgeGraphSDK:
    def __init__(self, name='213.0 Knowledge Graph SDK'): self.name=name
    def load(self):
        return {'ai':safe_json(AI_DASHBOARD),'learning':safe_json(LEARNING_DASHBOARD),'healing':safe_json(HEALING_DASHBOARD)}
    def build_context(self, raw):
        primary=raw.get('ai',{}) or {}
        secondary=raw.get('learning',{}) or {}
    ...
    def validate(self, context):
        ...
        decision='KNOWLEDGE GRAPH CONTEXT READY' if not errors else 'KNOWLEDGE GRAPH CONTEXT BLOCKED'
        return {'score':score,'decision':decision,'errors':errors,'warnings':warnings}
    def plan(self, context, validation):
        if validation['errors']: return {'mode':'PAUSED','operations':[],'message':'Knowledge Graph blocked by validation errors.'}
        operations=[]
        ...
        operations.append({'operation':'KNOWLEDGE_GRAPH_AUDIT_LOG','status':'PLANNED'})
        return {'mode':'CONTROLLED_KNOWLEDGE_GRAPH','operations':operations,'message':str(len(operations))+' graph operation planned.'}
    def export(self,payload,name=None):
        name=name or '213_0_knowledge_graph_sdk'
        ensure_dirs(KNOWLEDGE_GRAPH_DIR, REPORT_DIR, STATE_DIR); ts=now_stamp(); state=STATE_DIR/f'{name}_state_{ts}.json'; report=REPORT_DIR/f'{name}_raporu_{ts}.txt'
        write_json(KNOWLEDGE_GRAPH_SNAPSHOT,payload); write_json(state,payload); append_jsonl(KNOWLEDGE_GRAPH_HISTORY,payload)
        ...
        write_json(KNOWLEDGE_GRAPH_DASHBOARD,dash)
        lines=['='*80,'213.0 Knowledge Graph SDK'.upper(),'='*80,'Validation : '+str(payload['validation']['decision']),'Score      : '+str(payload['validation']['score']),'Mode       : '+str(payload['plan']['mode']),'Operations : '+str(len(payload['plan']['operations'])),'','Message:',str(payload['plan']['message']),'','Dosyalar:',str(KNOWLEDGE_GRAPH_SNAPSHOT),str(KNOWLEDGE_GRAPH_DASHBOARD),str(state),str(report)]
        report.write_text('\\n'.join(lines), encoding='utf-8')
        return {'snapshot':str(KNOWLEDGE_GRAPH_SNAPSHOT),'dashboard':str(KNOWLEDGE_GRAPH_DASHBOARD),'state':str(state),'report':str(report)}
    def run(self):
