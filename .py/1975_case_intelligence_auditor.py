...
MODULE_DIR = STATE / "case_graph_trainer" / "1975_case_intelligence_auditor"
MODULE_ID = "1975"
MODULE_NAME = "Case Intelligence Auditor"
def now_stamp():
...
def main():
    ...
    decision = "CASE INTELLIGENCE AUDITOR READY" if not val["errors"] else "CASE INTELLIGENCE AUDITOR BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1975_case_intelligence_auditor.json"
    state = MODULE_DIR / ("1975_case_intelligence_auditor_state_" + ts + ".json")
    report = REPORTS / ("1975_case_intelligence_auditor_raporu_" + ts + ".txt")
