...
STATE = BASE / "production_state"; REPORTS = BASE / "raporlar"; MODULE_DIR = STATE / "decision_processing_pipeline" / "1108_card_quality_gate"
MODULE_ID = "1108"; MODULE_NAME = "Card Quality Gate"
...
def main():
    ...
    decision = "CARD QUALITY GATE READY" if not val["errors"] else "CARD QUALITY GATE BLOCKED"
    ...
    ts = now_stamp(); output = MODULE_DIR / "1108_card_quality_gate.json"; state = MODULE_DIR / ("1108_card_quality_gate_state_" + ts + ".json"); report = REPORTS / ("1108_card_quality_gate_raporu_" + ts + ".txt")
