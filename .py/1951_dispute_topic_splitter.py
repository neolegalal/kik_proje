...
MODULE_DIR = STATE / "unified_decision_processor" / "1951_dispute_topic_splitter"
MODULE_ID = "1951"
MODULE_NAME = "Dispute Topic Splitter"
def now_stamp():
...
def main():
    ...
    decision = "DISPUTE TOPIC SPLITTER READY" if not val["errors"] else "DISPUTE TOPIC SPLITTER BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1951_dispute_topic_splitter.json"
    state = MODULE_DIR / ("1951_dispute_topic_splitter_state_" + ts + ".json")
    report = REPORTS / ("1951_dispute_topic_splitter_raporu_" + ts + ".txt")
