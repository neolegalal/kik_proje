...
FACTORY_DIR = STATE / "production_data_factory" / "502_decision_intake_engine"
MODULE_ID = "502"
MODULE_NAME = "Decision Intake Engine"
...
def analyze(payload):
    ...
    decision='DECISION INTAKE ENGINE READY' if not validation.get('errors') else 'DECISION INTAKE ENGINE REVIEW'
...
def main():
    ...
    ts=now_stamp(); output=FACTORY_DIR/('502_decision_intake_engine.json'); state=FACTORY_DIR/('502_decision_intake_engine_state_'+ts+'.json'); report=REPORTS/('502_decision_intake_engine_raporu_'+ts+'.txt')
