...
MODULE_DIR = STATE / "legal_advisory_intelligence" / "1305_outcome_probability_scorer"
MODULE_ID = "1305"
MODULE_NAME = "Outcome Probability Scorer"
...
def main():
    ...
    decision = "OUTCOME PROBABILITY SCORER READY" if not val["errors"] else "OUTCOME PROBABILITY SCORER BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1305_outcome_probability_scorer.json"
    state = MODULE_DIR / ("1305_outcome_probability_scorer_state_" + ts + ".json")
    report = REPORTS / ("1305_outcome_probability_scorer_raporu_" + ts + ".txt")
