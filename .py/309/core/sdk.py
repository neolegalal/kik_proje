...
class NeolegalPlatformOsSDK:
    def __init__(self, name="309.0 NeoLegal Platform OS SDK"):
        self.name = name
    ...
    def validate(self, context):
        ...
        decision="NEOLEGAL PLATFORM OS CONTEXT READY" if not errors else "NEOLEGAL PLATFORM OS CONTEXT BLOCKED"
        return {'score':score,'decision':decision,'errors':errors,'warnings':warnings}

    def plan(self, context, validation):
        if validation['errors']: return {'mode':'PAUSED','operations':[],'message':'NeoLegal Platform OS blocked by validation errors.'}
        operations=[]
        ...
        operations.append({'operation':'NEOLEGAL_PLATFORM_OS_AUDIT_LOG','status':'PLANNED'})
        return {'mode':'CONTROLLED_NEOLEGAL_PLATFORM_OS','operations':operations,'message':str(len(operations))+' neolegal platform os operation planned.'}

    def export(self, payload, name=None):
        name=name or '309_0_neolegal_platform_os_sdk'
        ensure_dirs(NEOLEGAL_PLATFORM_OS_DIR, REPORT_DIR, STATE_DIR)
        ts=now_stamp(); state=STATE_DIR/f'{name}_state_{ts}.json'; report=REPORT_DIR/f'{name}_raporu_{ts}.txt'
        write_json(NEOLEGAL_PLATFORM_OS_SNAPSHOT, payload); write_json(state, payload); append_jsonl(NEOLEGAL_PLATFORM_OS_HISTORY, payload)
        ...
        write_json(NEOLEGAL_PLATFORM_OS_DASHBOARD, dash)
        lines=['='*80,'309.0 NeoLegal Platform OS SDK'.upper(),'='*80,'Validation : '+str(payload['validation']['decision']),'Score      : '+str(payload['validation']['score']),'Mode       : '+str(payload['plan']['mode']),'Operations : '+str(len(payload['plan']['operations'])),'','Message:',str(payload['plan']['message']),'','Dosyalar:',str(NEOLEGAL_PLATFORM_OS_SNAPSHOT),str(NEOLEGAL_PLATFORM_OS_DASHBOARD),str(report)]
        report.write_text('\\n'.join(lines), encoding='utf-8')
        return {'snapshot':str(NEOLEGAL_PLATFORM_OS_SNAPSHOT),'dashboard':str(NEOLEGAL_PLATFORM_OS_DASHBOARD),'state':str(state),'report':str(report)}
