...
MODULE_DIR=BASE/"production_state"/"neolegal_legal_brain"/"2101_legal_memory"
...
def main():
    ...
    r=NeoLegalLegalBrainSDK(name="2101 Legal Memory",case_text=a.case_text,master_record_path=a.master_record,execute=a.execute).run()
    v=r["payload"]["validation"]; b=r["payload"]["brain_state"]; audit=r["payload"]["audit"]
    decision="LEGAL MEMORY READY" if not v["errors"] else "LEGAL MEMORY BLOCKED"
    ...
    stamp=ts(); out=MODULE_DIR/"2101_legal_memory.json"; state=MODULE_DIR/("2101_legal_memory_state_"+stamp+".json"); rep=REPORTS/("2101_legal_memory_raporu_"+stamp+".txt")
    payload={"module_id":"2101","module_name":"Legal Memory","analysis":analysis,"sdk_reference":r["paths"]}
    ...
    lines=["="*80,"2101 LEGAL MEMORY","="*80,"Score                 : "+str(analysis["score"])+" / 100","Decision              : "+decision,"Thinking Score        : "+str(analysis["thinking_score"])+" / 100","Base Probability      : "+str(analysis["base_probability"])+" / 100","Judge View            : "+str(analysis["judge_view"]),"Reflection Confidence : "+str(analysis["reflection_confidence"])+" / 100","Audit                 : "+str(analysis["audit"])]
