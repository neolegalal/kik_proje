...
MODULE_DIR = STATE / "client_workspace_memory" / "1703_conversation_memory_manager"
MODULE_ID = "1703"
MODULE_NAME = "Conversation Memory Manager"
def now_stamp():
...
def main():
    ...
    decision = "CONVERSATION MEMORY MANAGER READY" if not val["errors"] else "CONVERSATION MEMORY MANAGER BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1703_conversation_memory_manager.json"
    state = MODULE_DIR / ("1703_conversation_memory_manager_state_" + ts + ".json")
    report = REPORTS / ("1703_conversation_memory_manager_raporu_" + ts + ".txt")
