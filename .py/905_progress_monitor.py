...
MODULE_DIR = STATE / "production_master" / "905_progress_monitor"
MODULE_ID = "905"
MODULE_NAME = "Progress Monitor"
...
def main():
    ...
    score=val['score']; decision='PROGRESS MONITOR READY' if not val['errors'] else 'PROGRESS MONITOR BLOCKED'
    ...
    ts=now_stamp(); output=MODULE_DIR/('905_progress_monitor.json'); state=MODULE_DIR/('905_progress_monitor_state_'+ts+'.json'); report=REPORTS/('905_progress_monitor_raporu_'+ts+'.txt')
