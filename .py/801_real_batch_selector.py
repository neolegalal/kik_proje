...
MODULE_DIR = STATE / "real_production_engine" / "801_real_batch_selector"
MODULE_ID = "801"
MODULE_NAME = "Real Batch Selector"
...
def analyze(payload):
    ...
    decision='REAL BATCH SELECTOR READY' if not validation.get('errors') else 'REAL BATCH SELECTOR REVIEW'
...
def main():
    ...
    ts=now_stamp(); output=MODULE_DIR/('801_real_batch_selector.json'); state=MODULE_DIR/('801_real_batch_selector_state_'+ts+'.json'); report=REPORTS/('801_real_batch_selector_raporu_'+ts+'.txt')
