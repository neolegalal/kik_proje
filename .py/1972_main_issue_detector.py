...
MODULE_DIR = STATE / "case_graph_trainer" / "1972_main_issue_detector"
MODULE_ID = "1972"
MODULE_NAME = "Main Issue Detector"
def now_stamp():
...
def main():
    ...
    decision = "MAIN ISSUE DETECTOR READY" if not val["errors"] else "MAIN ISSUE DETECTOR BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1972_main_issue_detector.json"
    state = MODULE_DIR / ("1972_main_issue_detector_state_" + ts + ".json")
    report = REPORTS / ("1972_main_issue_detector_raporu_" + ts + ".txt")
