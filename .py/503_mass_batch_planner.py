...
FACTORY_DIR = STATE / "production_data_factory" / "503_mass_batch_planner"
MODULE_ID = "503"
MODULE_NAME = "Mass Batch Planner"
...
def analyze(payload):
    ...
    decision='MASS BATCH PLANNER READY' if not validation.get('errors') else 'MASS BATCH PLANNER REVIEW'
...
def main():
    ...
    ts=now_stamp(); output=FACTORY_DIR/('503_mass_batch_planner.json'); state=FACTORY_DIR/('503_mass_batch_planner_state_'+ts+'.json'); report=REPORTS/('503_mass_batch_planner_raporu_'+ts+'.txt')
