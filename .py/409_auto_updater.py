...
OS_DIR = STATE / "production_os" / "409_auto_updater"
MODULE_ID = "409"
MODULE_NAME = "Auto Updater"
...
def analyze(payload):
    ...
    decision = 'AUTO UPDATER READY' if not validation.get('errors') else 'AUTO UPDATER REVIEW'
...
def main():
    ...
    ts = now_stamp()
    output = OS_DIR / ('409_auto_updater.json')
    state = OS_DIR / ('409_auto_updater_state_' + ts + '.json')
    report = REPORTS / ('409_auto_updater_raporu_' + ts + '.txt')
