...
MODULE_DIR = STATE / "case_graph_trainer" / "1992_rag_dataset_builder"
MODULE_ID = "1992"
MODULE_NAME = "Rag Dataset Builder"
def now_stamp():
...
def main():
    ...
    decision = "RAG DATASET BUILDER READY" if not val["errors"] else "RAG DATASET BUILDER BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1992_rag_dataset_builder.json"
    state = MODULE_DIR / ("1992_rag_dataset_builder_state_" + ts + ".json")
    report = REPORTS / ("1992_rag_dataset_builder_raporu_" + ts + ".txt")
