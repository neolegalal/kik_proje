...
MODULE_DIR = STATE / "production_safety_gate" / "813_api_cost_safety_checker"
MODULE_ID = "813"
MODULE_NAME = "API Cost Safety Checker"
...
def main():
    ...
    decision='API COST SAFETY CHECKER READY' if final['decision'] in ('PASS','WARN') else 'API COST SAFETY CHECKER BLOCKED'
    ...
    ts=now_stamp(); output=MODULE_DIR/('813_api_cost_safety_checker.json'); state=MODULE_DIR/('813_api_cost_safety_checker_state_'+ts+'.json'); report=REPORTS/('813_api_cost_safety_checker_raporu_'+ts+'.txt')
