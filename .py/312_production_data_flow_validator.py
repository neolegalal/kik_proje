...
AUDIT_DIR = STATE / "production_readiness" / '312_production_data_flow_validator'
MODULE_ID = '312'
MODULE_NAME = 'Production Data Flow Validator'
VERSION = 'v2.19'
TAG = 'v2.19-production-data-flow-validator'
MUST_HAVE = ['168', '169', '170', '173']
PATTERNS = ['db', 'import', 'export', 'rag', 'kart', 'production']
...
def create_docs():
    ...
    release_path = RELEASES / 'v2.19-production-data-flow-validator.md'
...
def main():
    ...
    ts = now_stamp()
    state_path = AUDIT_DIR / (MODULE_ID + "_production_data_flow_validator_audit_" + ts + ".json")
    report_path = REPORTS / (MODULE_ID + "_production_data_flow_validator_raporu_" + ts + ".txt")
