...
MODULE_DIR = STATE / "case_graph_trainer" / "1984_contradiction_relation_builder"
MODULE_ID = "1984"
MODULE_NAME = "Contradiction Relation Builder"
def now_stamp():
...
def main():
    ...
    decision = "CONTRADICTION RELATION BUILDER READY" if not val["errors"] else "CONTRADICTION RELATION BUILDER BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1984_contradiction_relation_builder.json"
    state = MODULE_DIR / ("1984_contradiction_relation_builder_state_" + ts + ".json")
    report = REPORTS / ("1984_contradiction_relation_builder_raporu_" + ts + ".txt")
