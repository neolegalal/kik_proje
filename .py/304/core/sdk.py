...
class AuditIntelligenceSDK:
    def __init__(self, name="304.0 Audit Intelligence SDK"):
        self.name = name
    ...
    def validate(self, context):
        ...
        decision="AUDIT INTELLIGENCE CONTEXT READY" if not errors else "AUDIT INTELLIGENCE CONTEXT BLOCKED"
        return {'score':score,'decision':decision,'errors':errors,'warnings':warnings}

    def plan(self, context, validation):
        if validation['errors']: return {'mode':'PAUSED','operations':[],'message':'Audit Intelligence blocked by validation errors.'}
        operations=[]
        ...
        operations.append({'operation':'AUDIT_INTELLIGENCE_AUDIT_LOG','status':'PLANNED'})
        return {'mode':'CONTROLLED_AUDIT_INTELLIGENCE','operations':operations,'message':str(len(operations))+' audit intelligence operation planned.'}

    def export(self, payload, name=None):
        name=name or '304_0_audit_intelligence_sdk'
        ensure_dirs(AUDIT_INTELLIGENCE_DIR, REPORT_DIR, STATE_DIR)
        ts=now_stamp(); state=STATE_DIR/f'{name}_state_{ts}.json'; report=REPORT_DIR/f'{name}_raporu_{ts}.txt'
        write_json(AUDIT_INTELLIGENCE_SNAPSHOT, payload); write_json(state, payload); append_jsonl(AUDIT_INTELLIGENCE_HISTORY, payload)
        ...
        write_json(AUDIT_INTELLIGENCE_DASHBOARD, dash)
        lines=['='*80,'304.0 Audit Intelligence SDK'.upper(),'='*80,'Validation : '+str(payload['validation']['decision']),'Score      : '+str(payload['validation']['score']),'Mode       : '+str(payload['plan']['mode']),'Operations : '+str(len(payload['plan']['operations'])),'','Message:',str(payload['plan']['message']),'','Dosyalar:',str(AUDIT_INTELLIGENCE_SNAPSHOT),str(AUDIT_INTELLIGENCE_DASHBOARD),str(report)]
        report.write_text('\\n'.join(lines), encoding='utf-8')
        return {'snapshot':str(AUDIT_INTELLIGENCE_SNAPSHOT),'dashboard':str(AUDIT_INTELLIGENCE_DASHBOARD),'state':str(state),'report':str(report)}
