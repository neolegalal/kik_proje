...
MODULE_DIR = STATE / "production_safety_gate" / "820_production_launch_gate"
MODULE_ID = "820"
MODULE_NAME = "Production Launch Gate"
...
def main():
    ...
    decision='PRODUCTION LAUNCH GATE READY' if final['decision'] in ('PASS','WARN') else 'PRODUCTION LAUNCH GATE BLOCKED'
    ...
    ts=now_stamp(); output=MODULE_DIR/('820_production_launch_gate.json'); state=MODULE_DIR/('820_production_launch_gate_state_'+ts+'.json'); report=REPORTS/('820_production_launch_gate_raporu_'+ts+'.txt')
