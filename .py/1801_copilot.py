...
MODULE_DIR = STATE / "next_generation_neolegal" / "1801_copilot"
MODULE_ID = "1801"
MODULE_NAME = "NeoLegal Copilot"
def now_stamp():
...
def main():
    ...
    decision = "NEOLEGAL COPILOT READY" if not val["errors"] else "NEOLEGAL COPILOT BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1801_copilot.json"
    state = MODULE_DIR / ("1801_copilot_state_" + ts + ".json")
    report = REPORTS / ("1801_copilot_raporu_" + ts + ".txt")
