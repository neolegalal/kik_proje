...
MODULE_DIR = STATE / "production_master" / "903_worker_manager"
MODULE_ID = "903"
MODULE_NAME = "Worker Manager"
...
def main():
    ...
    score=val['score']; decision='WORKER MANAGER READY' if not val['errors'] else 'WORKER MANAGER BLOCKED'
    ...
    ts=now_stamp(); output=MODULE_DIR/('903_worker_manager.json'); state=MODULE_DIR/('903_worker_manager_state_'+ts+'.json'); report=REPORTS/('903_worker_manager_raporu_'+ts+'.txt')
