...
MODULE_DIR = STATE / "production_safety_gate" / "812_pipeline_safety_checker"
MODULE_ID = "812"
MODULE_NAME = "Pipeline Safety Checker"
...
def main():
    ...
    decision='PIPELINE SAFETY CHECKER READY' if final['decision'] in ('PASS','WARN') else 'PIPELINE SAFETY CHECKER BLOCKED'
    ...
    ts=now_stamp(); output=MODULE_DIR/('812_pipeline_safety_checker.json'); state=MODULE_DIR/('812_pipeline_safety_checker_state_'+ts+'.json'); report=REPORTS/('812_pipeline_safety_checker_raporu_'+ts+'.txt')
