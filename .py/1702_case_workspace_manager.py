...
MODULE_DIR = STATE / "client_workspace_memory" / "1702_case_workspace_manager"
MODULE_ID = "1702"
MODULE_NAME = "Case Workspace Manager"
def now_stamp():
...
def main():
    ...
    decision = "CASE WORKSPACE MANAGER READY" if not val["errors"] else "CASE WORKSPACE MANAGER BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1702_case_workspace_manager.json"
    state = MODULE_DIR / ("1702_case_workspace_manager_state_" + ts + ".json")
    report = REPORTS / ("1702_case_workspace_manager_raporu_" + ts + ".txt")
