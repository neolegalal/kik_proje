...
OS_DIR = STATE / "production_os" / "404_production_monitor"
MODULE_ID = "404"
MODULE_NAME = "Production Monitor"
...
def analyze(payload):
    ...
    decision = 'PRODUCTION MONITOR READY' if not validation.get('errors') else 'PRODUCTION MONITOR REVIEW'
...
def main():
    ...
    ts = now_stamp()
    output = OS_DIR / ('404_production_monitor.json')
    state = OS_DIR / ('404_production_monitor_state_' + ts + '.json')
    report = REPORTS / ('404_production_monitor_raporu_' + ts + '.txt')
