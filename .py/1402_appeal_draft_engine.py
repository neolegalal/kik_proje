...
MODULE_DIR = STATE / "litigation_intelligence" / "1402_appeal_draft_engine"
MODULE_ID = "1402"
MODULE_NAME = "Appeal Draft Engine"
def now_stamp():
...
def main():
    ...
    decision = "APPEAL DRAFT ENGINE READY" if not val["errors"] else "APPEAL DRAFT ENGINE BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1402_appeal_draft_engine.json"
    state = MODULE_DIR / ("1402_appeal_draft_engine_state_" + ts + ".json")
    report = REPORTS / ("1402_appeal_draft_engine_raporu_" + ts + ".txt")
