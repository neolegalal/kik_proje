...
MODULE_DIR = STATE / "production_master" / "908_production_auditor"
MODULE_ID = "908"
MODULE_NAME = "Production Auditor"
...
def main():
    ...
    score=val['score']; decision='PRODUCTION AUDITOR READY' if not val['errors'] else 'PRODUCTION AUDITOR BLOCKED'
    ...
    ts=now_stamp(); output=MODULE_DIR/('908_production_auditor.json'); state=MODULE_DIR/('908_production_auditor_state_'+ts+'.json'); report=REPORTS/('908_production_auditor_raporu_'+ts+'.txt')
