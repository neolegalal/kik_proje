# -*- coding: utf-8 -*-
"""
181 v10 - FINAL MASTER PRODUCTION CONTROLLER / RECOVERY-AWARE
...
def main():
    print("=" * 80)
    print("181 v10 - FINAL MASTER PRODUCTION CONTROLLER / RECOVERY-AWARE")
    print("=" * 80)

    run_tag = tag()
    limit, flags = parse_args()

    log_path = os.path.join(LOG_DIR, f"181_v10_final_master_controller_log_{run_tag}.txt")
    state_path = os.path.join(STATE_DIR, f"181_v10_final_master_controller_state_{run_tag}.json")
    rapor_path = os.path.join(RAPOR_DIR, f"181_v10_final_master_controller_raporu_{run_tag}.txt")
    ...
    # v10 recovery-aware final decision:
    # 172 veya 177 ilk denemede FAIL olsa bile, 191/185 sonrası tekrar kontrol PASS ise
    # ilk FAIL adımları bloklayıcı sayılmaz.
    recovered_steps = set()

    if ai_cleaning_summary and ai_cleaning_summary.get("final_status") in {"PASS_AFTER_191"}:
        recovered_steps.add("172")

    if healing_summary and healing_summary.get("final_status") in {"PASS_AFTER_185", "PASS_AFTER_185V2"}:
        recovered_steps.add("177")
    ...
    hard_ok = all(st.get("ok") for st in hard_steps)
    ...
    with open(rapor_path, "w", encoding="utf-8") as f:
        f.write("181 v10 - FINAL MASTER PRODUCTION CONTROLLER / RECOVERY-AWARE RAPORU\n")
        f.write("=" * 80 + "\n\n")
    ...
    print("\n181 v10 FINAL MASTER CONTROLLER TAMAMLANDI")
    print("-" * 80)
