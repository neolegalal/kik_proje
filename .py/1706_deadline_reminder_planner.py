...
MODULE_DIR = STATE / "client_workspace_memory" / "1706_deadline_reminder_planner"
MODULE_ID = "1706"
MODULE_NAME = "Deadline Reminder Planner"
def now_stamp():
...
def main():
    ...
    decision = "DEADLINE REMINDER PLANNER READY" if not val["errors"] else "DEADLINE REMINDER PLANNER BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1706_deadline_reminder_planner.json"
    state = MODULE_DIR / ("1706_deadline_reminder_planner_state_" + ts + ".json")
    report = REPORTS / ("1706_deadline_reminder_planner_raporu_" + ts + ".txt")
