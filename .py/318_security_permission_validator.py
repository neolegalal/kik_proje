...
AUDIT_DIR = STATE / "production_readiness" / '318_security_permission_validator'
MODULE_ID = '318'
MODULE_NAME = 'Security & Permission Validator'
VERSION = 'v2.25'
TAG = 'v2.25-security-permission-validator'
MUST_HAVE = ['303', '304']
PATTERNS = ['security', 'permission', 'access', 'governance', 'audit']
...
def create_docs():
    ...
    release_path = RELEASES / 'v2.25-security-permission-validator.md'
...
def main():
    ...
    ts = now_stamp()
    state_path = AUDIT_DIR / (MODULE_ID + "_security_permission_validator_audit_" + ts + ".json")
    report_path = REPORTS / (MODULE_ID + "_security_permission_validator_raporu_" + ts + ".txt")
