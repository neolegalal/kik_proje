...
MODULE_DIR = EXECUTION_DIR / "207_1_batch_executor"
OUTPUT_FILE = MODULE_DIR / "207_1_batch_executor.json"

class BatchExecutorModule:
    def __init__(self):
        self.sdk = ExecutionSDK(name="207.1 Batch Executor")

    def analyze(self, context, plan):
        return {
            "module_id": "207.1",
            "module_name": "Batch Executor",
    ...
    def run(self):
        ...
        result = {
            ...
            "decision": "BATCH EXECUTOR READY" if not validation["errors"] else "BATCH EXECUTOR REVIEW",
            "risk": context.get("scheduler_risk"),
            "recommendation": plan.get("message")
        }

        payload = {
            "module": "207.1 Batch Executor",
            "created_at": now_text(),
            "analysis": analysis,
            "result": result,
        ...
        state = STATE_DIR / f"207_1_batch_executor_state_{ts}.json"
        report = REPORT_DIR / f"207_1_batch_executor_raporu_{ts}.txt"
