...
MODULE_DIR = EXECUTION_DIR / "207_2_worker_dispatcher"
OUTPUT_FILE = MODULE_DIR / "207_2_worker_dispatcher.json"

class WorkerDispatcherModule:
    def __init__(self):
        self.sdk = ExecutionSDK(name="207.2 Worker Dispatcher")

    def analyze(self, context, plan):
        return {
            "module_id": "207.2",
            "module_name": "Worker Dispatcher",
    ...
    def run(self):
        ...
        result = {
            ...
            "decision": "WORKER DISPATCHER READY" if not validation["errors"] else "WORKER DISPATCHER REVIEW",
            "risk": context.get("scheduler_risk"),
            "recommendation": plan.get("message")
        }

        payload = {
            "module": "207.2 Worker Dispatcher",
            "created_at": now_text(),
            "analysis": analysis,
            "result": result,
        ...
        state = STATE_DIR / f"207_2_worker_dispatcher_state_{ts}.json"
        report = REPORT_DIR / f"207_2_worker_dispatcher_raporu_{ts}.txt"
