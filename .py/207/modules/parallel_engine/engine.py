...
MODULE_DIR = EXECUTION_DIR / "207_6_parallel_engine"
OUTPUT_FILE = MODULE_DIR / "207_6_parallel_engine.json"

class ParallelEngineModule:
    def __init__(self):
        self.sdk = ExecutionSDK(name="207.6 Parallel Engine")

    def analyze(self, context, plan):
        return {
            "module_id": "207.6",
            "module_name": "Parallel Engine",
    ...
    def run(self):
        ...
        result = {
            ...
            "decision": "PARALLEL ENGINE READY" if not validation["errors"] else "PARALLEL ENGINE REVIEW",
            "risk": context.get("scheduler_risk"),
            "recommendation": plan.get("message")
        }

        payload = {
            "module": "207.6 Parallel Engine",
            "created_at": now_text(),
            "analysis": analysis,
            "result": result,
        ...
        state = STATE_DIR / f"207_6_parallel_engine_state_{ts}.json"
        report = REPORT_DIR / f"207_6_parallel_engine_raporu_{ts}.txt"
