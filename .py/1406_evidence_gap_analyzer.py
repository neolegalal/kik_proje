...
MODULE_DIR = STATE / "litigation_intelligence" / "1406_evidence_gap_analyzer"
MODULE_ID = "1406"
MODULE_NAME = "Evidence Gap Analyzer"
def now_stamp():
...
def main():
    ...
    decision = "EVIDENCE GAP ANALYZER READY" if not val["errors"] else "EVIDENCE GAP ANALYZER BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1406_evidence_gap_analyzer.json"
    state = MODULE_DIR / ("1406_evidence_gap_analyzer_state_" + ts + ".json")
    report = REPORTS / ("1406_evidence_gap_analyzer_raporu_" + ts + ".txt")
