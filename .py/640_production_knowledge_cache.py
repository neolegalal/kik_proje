...
IPE_DIR = STATE / "intelligent_production_engine" / "640_production_knowledge_cache"
MODULE_ID = "640"
MODULE_NAME = "Production Knowledge Cache"
...
def analyze(payload):
    ...
    decision='PRODUCTION KNOWLEDGE CACHE READY' if not validation.get('errors') else 'PRODUCTION KNOWLEDGE CACHE REVIEW'
...
def main():
    ...
    ts=now_stamp(); output=IPE_DIR/('640_production_knowledge_cache.json'); state=IPE_DIR/('640_production_knowledge_cache_state_'+ts+'.json'); report=REPORTS/('640_production_knowledge_cache_raporu_'+ts+'.txt')
