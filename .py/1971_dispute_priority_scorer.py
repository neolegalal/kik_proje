...
MODULE_DIR = STATE / "case_graph_trainer" / "1971_dispute_priority_scorer"
MODULE_ID = "1971"
MODULE_NAME = "Dispute Priority Scorer"
def now_stamp():
...
def main():
    ...
    decision = "DISPUTE PRIORITY SCORER READY" if not val["errors"] else "DISPUTE PRIORITY SCORER BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1971_dispute_priority_scorer.json"
    state = MODULE_DIR / ("1971_dispute_priority_scorer_state_" + ts + ".json")
    report = REPORTS / ("1971_dispute_priority_scorer_raporu_" + ts + ".txt")
