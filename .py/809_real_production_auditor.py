...
MODULE_DIR = STATE / "real_production_engine" / "809_real_production_auditor"
MODULE_ID = "809"
MODULE_NAME = "Real Production Auditor"
...
def analyze(payload):
    ...
    decision='REAL PRODUCTION AUDITOR READY' if not validation.get('errors') else 'REAL PRODUCTION AUDITOR REVIEW'
...
def main():
    ...
    ts=now_stamp(); output=MODULE_DIR/('809_real_production_auditor.json'); state=MODULE_DIR/('809_real_production_auditor_state_'+ts+'.json'); report=REPORTS/('809_real_production_auditor_raporu_'+ts+'.txt')
