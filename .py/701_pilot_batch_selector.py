...
PILOT_MODULE_DIR = STATE / "pilot_production" / "701_pilot_batch_selector"
MODULE_ID = "701"
MODULE_NAME = "Pilot Batch Selector"
...
def analyze(payload):
    ...
    decision='PILOT BATCH SELECTOR READY' if not validation.get('errors') else 'PILOT BATCH SELECTOR REVIEW'
...
def main():
    ...
    ts=now_stamp(); output=PILOT_MODULE_DIR/('701_pilot_batch_selector.json'); state=PILOT_MODULE_DIR/('701_pilot_batch_selector_state_'+ts+'.json'); report=REPORTS/('701_pilot_batch_selector_raporu_'+ts+'.txt')
