...
MODULE_DIR = STATE / "case_graph_trainer" / "1995_ai_trainer_certificate"
MODULE_ID = "1995"
MODULE_NAME = "AI Trainer Certificate"
def now_stamp():
...
def main():
    ...
    decision = "AI TRAINER CERTIFICATE READY" if not val["errors"] else "AI TRAINER CERTIFICATE BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1995_ai_trainer_certificate.json"
    state = MODULE_DIR / ("1995_ai_trainer_certificate_state_" + ts + ".json")
    report = REPORTS / ("1995_ai_trainer_certificate_raporu_" + ts + ".txt")
