...
MODULE_DIR = STATE / "neolegal_expert_orchestrator" / "1605_legal_reasoning_orchestrator"
MODULE_ID = "1605"
MODULE_NAME = "Legal Reasoning Orchestrator"
def now_stamp():
...
def main():
    ...
    decision = "LEGAL REASONING ORCHESTRATOR READY" if not val["errors"] else "LEGAL REASONING ORCHESTRATOR BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1605_legal_reasoning_orchestrator.json"
    state = MODULE_DIR / ("1605_legal_reasoning_orchestrator_state_" + ts + ".json")
    report = REPORTS / ("1605_legal_reasoning_orchestrator_raporu_" + ts + ".txt")
