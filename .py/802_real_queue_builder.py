...
MODULE_DIR = STATE / "real_production_engine" / "802_real_queue_builder"
MODULE_ID = "802"
MODULE_NAME = "Real Queue Builder"
...
def analyze(payload):
    ...
    decision='REAL QUEUE BUILDER READY' if not validation.get('errors') else 'REAL QUEUE BUILDER REVIEW'
...
def main():
    ...
    ts=now_stamp(); output=MODULE_DIR/('802_real_queue_builder.json'); state=MODULE_DIR/('802_real_queue_builder_state_'+ts+'.json'); report=REPORTS/('802_real_queue_builder_raporu_'+ts+'.txt')
