...
MODULE_DIR = STATE / "next_generation_neolegal" / "1808_next_generation_dashboard"
MODULE_ID = "1808"
MODULE_NAME = "Next Generation Dashboard"
def now_stamp():
...
def main():
    ...
    decision = "NEXT GENERATION DASHBOARD READY" if not val["errors"] else "NEXT GENERATION DASHBOARD BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1808_next_generation_dashboard.json"
    state = MODULE_DIR / ("1808_next_generation_dashboard_state_" + ts + ".json")
    report = REPORTS / ("1808_next_generation_dashboard_raporu_" + ts + ".txt")
