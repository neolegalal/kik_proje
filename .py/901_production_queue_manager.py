...
MODULE_DIR = STATE / "production_master" / "901_production_queue_manager"
MODULE_ID = "901"
MODULE_NAME = "Production Queue Manager"
...
def main():
    ...
    score=val['score']; decision='PRODUCTION QUEUE MANAGER READY' if not val['errors'] else 'PRODUCTION QUEUE MANAGER BLOCKED'
    ...
    ts=now_stamp(); output=MODULE_DIR/('901_production_queue_manager.json'); state=MODULE_DIR/('901_production_queue_manager_state_'+ts+'.json'); report=REPORTS/('901_production_queue_manager_raporu_'+ts+'.txt')
