...
MODULE_DIR = AUTOMATION_DIR / "208_3_safe_run_gate"
OUTPUT_FILE = MODULE_DIR / "208_3_safe_run_gate.json"

class SafeRunGateModule:
    def __init__(self):
        self.sdk = AutomationSDK(name="208.3 Safe Run Gate")

    def analyze(self, context, plan):
        return {
            "module_id": "208.3",
            "module_name": "Safe Run Gate",
    ...
    def run(self):
        ...
        result = {
            ...
            "decision": "SAFE RUN GATE READY" if not validation["errors"] else "SAFE RUN GATE REVIEW",
            "risk": context.get("risk"),
            "recommendation": plan.get("message")
        }

        payload = {
            "module": "208.3 Safe Run Gate",
            "created_at": now_text(),
            "analysis": analysis,
            "result": result,
        ...
        state = STATE_DIR / f"208_3_safe_run_gate_state_{ts}.json"
        report = REPORT_DIR / f"208_3_safe_run_gate_raporu_{ts}.txt"
