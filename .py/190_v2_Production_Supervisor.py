...
def recommendation(health_score, states):
    ...
    if not ready181:
        return "181 son üretim hazır değil. Önce 181 v8/v9 ile auto-clean + self-healing akışı kontrol edilmeli."
    if fail_cards > 0:
...
def main():
    print("=" * 80)
    print("190 v2 - PRODUCTION SUPERVISOR")
    print("=" * 80)
    ...
    if not states["181"]:
        for old_prefix in [
            ...
            path, st = latest_state(old_prefix)
            if st:
                states["181"] = st
    ...
    txt_path = os.path.join(RAPOR_DIR, f"190_v2_production_supervisor_raporu_{run_tag}.txt")
    state_path = os.path.join(STATE_DIR, f"190_v2_production_supervisor_state_{run_tag}.json")
    summary_path = os.path.join(SUPERVISOR_DIR, f"190_v2_supervisor_summary_{run_tag}.json")
    ...
    with open(txt_path, "w", encoding="utf-8") as f:
        f.write("190 v2 - PRODUCTION SUPERVISOR RAPORU\n")
        f.write("=" * 80 + "\n\n")
    ...
    print("\n190 v2 PRODUCTION SUPERVISOR TAMAMLANDI")
    print("-" * 80)
