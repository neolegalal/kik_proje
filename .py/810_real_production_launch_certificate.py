...
MODULE_DIR = STATE / "real_production_engine" / "810_real_production_launch_certificate"
MODULE_ID = "810"
MODULE_NAME = "Real Production Launch Certificate"
...
def analyze(payload):
    ...
    decision='REAL PRODUCTION LAUNCH CERTIFICATE READY' if not validation.get('errors') else 'REAL PRODUCTION LAUNCH CERTIFICATE REVIEW'
...
def main():
    ...
    ts=now_stamp(); output=MODULE_DIR/('810_real_production_launch_certificate.json'); state=MODULE_DIR/('810_real_production_launch_certificate_state_'+ts+'.json'); report=REPORTS/('810_real_production_launch_certificate_raporu_'+ts+'.txt')
