...
OS_DIR = STATE / "production_os" / "401_production_console"
MODULE_ID = "401"
MODULE_NAME = "Production Console"
...
def analyze(payload):
    ...
    decision = 'PRODUCTION CONSOLE READY' if not validation.get('errors') else 'PRODUCTION CONSOLE REVIEW'
...
def main():
    ...
    ts = now_stamp()
    output = OS_DIR / ('401_production_console.json')
    state = OS_DIR / ('401_production_console_state_' + ts + '.json')
    report = REPORTS / ('401_production_console_raporu_' + ts + '.txt')
