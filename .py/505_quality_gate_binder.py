...
FACTORY_DIR = STATE / "production_data_factory" / "505_quality_gate_binder"
MODULE_ID = "505"
MODULE_NAME = "Quality Gate Binder"
...
def analyze(payload):
    ...
    decision='QUALITY GATE BINDER READY' if not validation.get('errors') else 'QUALITY GATE BINDER REVIEW'
...
def main():
    ...
    ts=now_stamp(); output=FACTORY_DIR/('505_quality_gate_binder.json'); state=FACTORY_DIR/('505_quality_gate_binder_state_'+ts+'.json'); report=REPORTS/('505_quality_gate_binder_raporu_'+ts+'.txt')
