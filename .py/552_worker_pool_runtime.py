...
RUNTIME_DIR = STATE / "production_factory_runtime" / "552_worker_pool_runtime"
MODULE_ID = "552"
MODULE_NAME = "Worker Pool Runtime"
...
def analyze(payload):
    ...
    decision='WORKER POOL RUNTIME READY' if not validation.get('errors') else 'WORKER POOL RUNTIME REVIEW'
...
def main():
    ...
    ts=now_stamp(); output=RUNTIME_DIR/('552_worker_pool_runtime.json'); state=RUNTIME_DIR/('552_worker_pool_runtime_state_'+ts+'.json'); report=REPORTS/('552_worker_pool_runtime_raporu_'+ts+'.txt')
