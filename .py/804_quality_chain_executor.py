...
MODULE_DIR = STATE / "real_production_engine" / "804_quality_chain_executor"
MODULE_ID = "804"
MODULE_NAME = "Quality Chain Executor"
...
def analyze(payload):
    ...
    decision='QUALITY CHAIN EXECUTOR READY' if not validation.get('errors') else 'QUALITY CHAIN EXECUTOR REVIEW'
...
def main():
    ...
    ts=now_stamp(); output=MODULE_DIR/('804_quality_chain_executor.json'); state=MODULE_DIR/('804_quality_chain_executor_state_'+ts+'.json'); report=REPORTS/('804_quality_chain_executor_raporu_'+ts+'.txt')
