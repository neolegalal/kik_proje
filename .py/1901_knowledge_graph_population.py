...
MODULE_DIR = STATE / "neolegal_evolution_platform" / "1901_knowledge_graph_population"
MODULE_ID = "1901"
MODULE_NAME = "Knowledge Graph Population"
def now_stamp():
...
def main():
    ...
    decision = "KNOWLEDGE GRAPH POPULATION READY" if not val["errors"] else "KNOWLEDGE GRAPH POPULATION BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1901_knowledge_graph_population.json"
    state = MODULE_DIR / ("1901_knowledge_graph_population_state_" + ts + ".json")
    report = REPORTS / ("1901_knowledge_graph_population_raporu_" + ts + ".txt")
