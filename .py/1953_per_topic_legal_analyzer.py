...
MODULE_DIR = STATE / "unified_decision_processor" / "1953_per_topic_legal_analyzer"
MODULE_ID = "1953"
MODULE_NAME = "Per Topic Legal Analyzer"
def now_stamp():
...
def main():
    ...
    decision = "PER TOPIC LEGAL ANALYZER READY" if not val["errors"] else "PER TOPIC LEGAL ANALYZER BLOCKED"
    ...
    ts = now_stamp()
    output = MODULE_DIR / "1953_per_topic_legal_analyzer.json"
    state = MODULE_DIR / ("1953_per_topic_legal_analyzer_state_" + ts + ".json")
    report = REPORTS / ("1953_per_topic_legal_analyzer_raporu_" + ts + ".txt")
